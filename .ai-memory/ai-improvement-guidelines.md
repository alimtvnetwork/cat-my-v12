...
**Source:** Deep read of `src/`, `BE/`, `app/`, `02-spec/`, `.ai-memory/`, recent plans (86, 88, 90, 96, 98), and CHANGELOG  
...
Treat this as **binding guidance** alongside `.ai-memory/coding-guidelines.md` and `02-spec/02-coding-guidelines/`. When they conflict, the stricter rule wins.
...
`.ai-memory/strictly-avoid.md` and Plan 43/91 enforced `*Type` suffix enums everywhere. Real registries live in `src/lib/constants/` and `src/types/`.
...
**Evidence:** Shell spec describes Tauri; code uses Chromium MV3. Some `02-spec/21-app/` docs still say "spec only, no code."

**Improvement for AI:**

1. Read `02-spec/21-app/shell/03-implementation-status.md` before shell work
...
4. Active plan in `.ai-memory/plans/pending/` OR completed plan for your area
5. Relevant spec under `02-spec/21-app/` or `02-spec/03-error-manage/`
...
- Functions ≤ 15 lines per `02-spec/coding-guidelines/python.md`
...
When picking up backlog items, create or extend a plan in `.ai-memory/plans/pending/` — do not drive-by refactor without plan coverage.

---

## Part G — Anti-patterns observed in past AI sessions

From `.ai-memory/memory/workflow/`, strictly-avoid.md, and CHANGELOG "not fixed (deliberate)" notes:
