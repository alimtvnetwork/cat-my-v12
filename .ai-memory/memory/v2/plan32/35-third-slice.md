...
- Prior-slice memos (`00-pattern-edge-baseline.md`, `20-first-slice.md`, `30-second-slice-closeout.md`) do not exist on disk. This memo names slice 3 explicitly and does not claim their history. Plan chain files (`.ai-memory/plans/pending/{32,53,54,55}-*.md`) stay in `pending/` because moving them to `completed/` without the prior memos would misrepresent the history.
