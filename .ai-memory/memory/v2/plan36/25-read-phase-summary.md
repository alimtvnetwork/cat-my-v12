...
`.ai-memory/plans/pending/61-plan36-app-shell-execution-slice-1.md` becomes the
executor once the user confirms whether `src-v3/` should be provided or the
consolidation path is preferred.

## Verification

- Five memo files written under `.ai-memory/memory/v2/plan36/` (00, 10, 15, 20, 25).
- `ls src-v3` confirmed missing; recorded in `15-v3-inventory.md`.
- `git diff --stat` scope: only `.ai-memory/memory/v2/plan36/*`, `package.json`,
