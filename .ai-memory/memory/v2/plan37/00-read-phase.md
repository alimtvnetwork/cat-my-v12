...
- `src/components/hmi/Titlebar.tsx` mounts a `<Link to="/" aria-label="Home">`
  ...
  `HmiShell` yet (cross-ref: `.ai-memory/memory/v2/plan36/10-shell-inventory.md`).
...
- Step 1: three reference screenshots are not persisted under
  `.ai-memory/02-spec/references/` or an equivalent index. Cannot proceed without the
...
- Step 12: plan is not moved to `.ai-memory/plans/done/` because steps 1, 6, 8,
  9, 11 remain open.
