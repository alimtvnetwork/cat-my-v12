...
- `.ai-memory/coding-guidelines.md`: hard function/file size caps, no nested ifs beyond depth 2, no `any`, enums live in dedicated files, error handling must surface (never swallow), naming: PascalCase components, camelCase functions, kebab-case files where noted.
- `.ai-memory/memory/03-error-manage.md` + `02-spec/03-error-manage/**`: every error path must log with correlation id, surface via `errorStore` and `GlobalErrorModal`, no silent `try/catch`. Toasts for transient worker health only.
- `.ai-memory/memory/04-design-system.md` (Plan 73 closeout additions):
...
- Plan 73 closeout (`.ai-memory/memory/v2/plan73/90-closeout.md`): 10 issues closed; deferred set is exactly {09, 11, 12, 13, 14, 15}. No scope creep in Plan 75.
