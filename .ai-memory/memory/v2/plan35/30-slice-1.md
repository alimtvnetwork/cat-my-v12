...
Source: `.ai-memory/memory/v2/plan35/25-read-phase-summary.md` line 15 (top gap: "Spec docs: `02-spec/21-app/**` narrative for Layers-vs-Properties contract + drag/drop/group/merge, Plan 35 steps 24-25, docs-only, no runtime risk").

## Chosen gap

Docs-only pass on `02-spec/21-app/31-rule-setup-screen.md` §10 to lock down which panel owns rule selection. Existing §10.1-§10.5 already cover ordering, membership, DnD, group/ungroup/merge, and error codes; the missing clause is selection ownership between Layers Panel (left rail) and Properties Panel (right rail).

## Target file

- `02-spec/21-app/31-rule-setup-screen.md` §10.6 (appended).
