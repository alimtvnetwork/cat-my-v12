...
- Step 1: wire codes registered in `02-spec/21-app/40-error-manage.md:131-133` and `02-spec/03-error-manage/98-changelog.md:13`. Confirmed by grep.
...
- Step 24-25: spec files that describe the Layers-vs-Properties contract and drag/drop/group/merge narrative in `02-spec/21-app/**` not yet updated (only the error registry rows landed). Kept open.
