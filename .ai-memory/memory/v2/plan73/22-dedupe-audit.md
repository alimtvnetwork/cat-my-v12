...
`.ai-memory/issues/22-duplicate-header-still-present.md` already reads `Status: closed` (Plan 66 SH-01, 2026-07-17). Invariant guard `tests/e2e/playwright_single_header.py` (44 lines) asserts exactly one `header[data-app-shell="true"]` across `/`, `/setup`, `/setup/rules`, `/projects`, `/run`, `/trial-run`. No further code change.
