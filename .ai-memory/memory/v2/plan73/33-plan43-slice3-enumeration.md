...
description: Read of `.ai-memory/plans/pending/46-plan43-execution-slice-3.md`; enumerates remaining code-quality sweep work before executing steps 34-38
type: reference
---

Source: `.ai-memory/plans/pending/46-plan43-execution-slice-3.md` (5 steps).

Remaining items mapped to Plan 73 steps 34-38:

1. Plan 73 step 34 <- 46 step 1 (partial): magic-string enumeration via `scripts/check-magic-strings.sh`, write to `.ai-memory/memory/v2/plan73/10-magic-strings.md`. Prereq: confirm the script exists (`ls scripts/check-magic-strings.sh`); if absent, either author it or fall back to `rg` over `src/` for literal role/severity/status strings.
...
3. Plan 73 step 36 <- boolean-arg -> enum object per `.ai-memory/02-spec/commands/21-code-quality-boolean-and-flow.md`. Top 10 call sites. Command file expected to still be `Status: pending` -> flip to `shipped` at step 38.
4. Plan 73 step 37 <- 46 step 1: PascalCase rename table under `.ai-memory/plans/subtasks/43-coding-quality-error-dialog-and-mode-flag/audit-findings.md`. Rules: no `interface I[A-Z]`, no `type .*Type =`, inline union types moved to named types under `src/lib/constants/`.
...
- 46 step 2: spec sync under `02-spec/02-coding-guidelines`, `02-spec/03-error-manage`, `02-spec/21-app`, `02-spec/24-app-ui-design-system` for any renamed identifiers.
