...
Issue: `.ai-memory/issues/26-ui-seed-values-not-facaded.md` (Status: open).
...
`SAMPLE_LIBRARY`, `SAMPLE_POV_MAP`, and `MOCK_TOOLS` are hardcoded ES module constants imported directly by HMI / editor components, so swapping the seed source (JSON now, remote later) still requires editing UI files, violating `02-spec/21-app/52-sdk-facade-pattern.md`.
