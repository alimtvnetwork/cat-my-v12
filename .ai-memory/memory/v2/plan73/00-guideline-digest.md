...
description: Distilled rules from coding-guidelines and 02-spec/03-error-manage that gate every step in Plan 73.
type: reference
---
...
- `.ai-memory/memory/03-error-manage.md`
- `.ai-memory/memory/22-coding-guidelines-digest.md`
- `.ai-memory/memory/24-coding-and-error-rulebook.md`
- `02-spec/03-error-manage/00-overview.md`, `structure.md`, `97-acceptance-criteria.md`, `98-changelog.md`, `99-consistency-report.md`
- `02-spec/03-error-manage/01-error-resolution/`, `02-error-architecture/`, `03-error-code-registry/`
...
11. `02-spec/03-error-manage/` overrides on any error path, no exceptions.
...
- Boolean function args become enums per `.ai-memory/02-spec/commands/21-code-quality-boolean-and-flow.md`.

## Enforcement in Plan 73

- Every UI fix step must run `bunx tsgo --noEmit` and `bunx vitest run` before flipping an issue to `closed`.
- Any error surface change (steps 28 and 29) reads the applicable file under `02-spec/03-error-manage/02-error-architecture/` and cites it in the commit message.
