...
Source of truth: `.ai-memory/memory/v2/plan29/20-windows.json` (rebuild with
`PYTHONPATH=. python3 scripts/security/plan29_windows.py --jsonl <path> --out
.ai-memory/memory/v2/plan29/20-windows.json`; verify with `--check`).
...
export at `.ai-memory/plans/subtasks/29-denial-burst-threshold-tuning/evidence/90d.jsonl`
...
PYTHONPATH=. python3 scripts/security/plan29_windows.py \
  ...
  --check --out .ai-memory/memory/v2/plan29/20-windows.json
```
