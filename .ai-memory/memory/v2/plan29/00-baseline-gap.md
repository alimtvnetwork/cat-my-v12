...
|                                                                                                                           8-11 (evidence CLI, tradeoff report) | landed         | `scripts/security/denial_evidence_cli.py:1-151`, `scripts/security/tradeoff_report.py`, `.ai-memory/plans/subtasks/29-denial-burst-threshold-tuning/04-candidate-evaluation.md`                                                                          |
|                                                                                                                                    12-15 (rollout status memo) | landed         | `.ai-memory/plans/subtasks/29-denial-burst-threshold-tuning/03-steps-12-15-status.md`                                                                                                                                                                    |
|                                                                                                                   16-19 (evidence spec 69a + synthetic corpus) | landed         | `02-spec/21-app/69a-v2-denial-tuning-evidence.md`                                                                                                                                                                                                         |
...
|                                Plan 33 slice 2 (getDenialBurstWindow server-fn + exporter `--percentiles` extension + windows.json snapshot + derivation memo) | open           | Plan file: `.ai-memory/plans/pending/48-plan33-server-fn-and-percentiles.md`                                                                                                                                                                             |
|                                                                                                                                  Field 90-day replay to unpark | open (blocked) | `evidence/90d.jsonl` at `.ai-memory/plans/subtasks/29-denial-burst-threshold-tuning/evidence/` is gitignored and empty locally. Cannot proceed without a real export.                                                                                    |

## Files read for this baseline

- Parent plan: `.ai-memory/plans/pending/29-denial-burst-threshold-tuning.md`
- Subtasks: `.ai-memory/plans/subtasks/29-denial-burst-threshold-tuning/{01,02,03,04,05}-*.md`
- Chain plan: `.ai-memory/plans/pending/33-plan-29-denial-burst-tuning-read-phase.md`
- This-slice plan: `.ai-memory/plans/pending/47-plan33-read-phase-kickoff.md`
- Next-slice plan: `.ai-memory/plans/pending/48-plan33-server-fn-and-percentiles.md`

## Appendix A: current `E_SEC_DENIAL_BURST` contract

From `02-spec/21-app/40-error-manage.md` A.1 (Security) and `app/core/security/audit_sink.py:31`:
...
- Does NOT change any code under `app/`, `src/`, `scripts/`, `tests/`, or `02-spec/`.
