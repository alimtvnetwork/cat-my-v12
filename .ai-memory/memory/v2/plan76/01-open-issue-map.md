...
description: Truly-open issues in .ai-memory/issues/ after Plan 75 closeout; scope correction for Plan 76.
type: reference
---
...
For each file in `.ai-memory/issues/*.md`, read the first `Status:` frontmatter line (line 1-6 range only). Files with multiple concatenated `Status:` lines further down (e.g. issue 17 file bundles 17+18+19 body drafts) do not count as open unless the top-of-file status is `open`.

## Truly open (1)

| #   | slug                               | severity | blocker                                                                                       |
| --- | ---------------------------------- | -------- | --------------------------------------------------------------------------------------------- |
| 16  | project-section-create-flow-broken | high     | blocked-on-user-answers (see `.ai-memory/ambiguity-questions/01-ui-v2-open-questions.md` Q1-Q7) |
...
Original Plan 76 assumed issues 10 and 17 needed fixes. Both are already closed. The only remaining `Status: open` is 16, which is explicitly parked pending user answers to `.ai-memory/ambiguity-questions/01-ui-v2-open-questions.md`.
...
- Split the multi-issue file `.ai-memory/issues/17-menu-hover-jitter-and-padding.md` so 18/19 drafts live in their own files with correct `Status:` values.
