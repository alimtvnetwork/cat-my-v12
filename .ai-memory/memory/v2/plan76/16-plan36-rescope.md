...
Edited `.ai-memory/plans/pending/36-ui-app-shell-and-src-v3-port.md` frontmatter to insert a "Rescope note (2026-07-18, Plan 76 SS-03)" block after the existing Revision note. The note documents:
...
`grep -n "Rescope note (2026-07-18, Plan 76 SS-03)" .ai-memory/plans/pending/36-ui-app-shell-and-src-v3-port.md` => hit at line ~13. Original 50-step body preserved intact below the note.

## Impact

Downstream slice plans (61/62/63) now inherit an accurate umbrella context. The next triage pass over `.ai-memory/plans/pending/` will not misread Plan 36 as "50 steps to execute".
