...
- **Rule Enforcement Memory Update**: Updated `.ai-memory/memory/24-coding-and-error-rulebook.md` to strictly enforce:
...
- Resurrect the TS Enum and Boolean refactoring via sub-agents or loop. Plan is tracked in `.ai-memory/plans/pending/89-enums-boolean-refactor.md`.

## Learned

- Subagents must be treated as ephemeral; their state needs persistent tracking in `.ai-memory/` to recover from server restarts.
- The Git index was heavily polluted with `__pycache__` which required a recursive purge.

## 🚫 Avoid, [Scattered Try/Catch]

- Avoid scattered try/catch blocks for database/IPC queries. Use the centralized query wrapper for automatic failure logging. See `.ai-memory/memory/24-coding-and-error-rulebook.md`.
