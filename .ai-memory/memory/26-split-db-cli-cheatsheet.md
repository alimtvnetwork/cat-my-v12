...
**Sources:** `02-spec/05-split-db-architecture/00-overview.md`, `01-fundamentals.md`, `02-features/01-cli-examples.md`, `02-spec/04-database-conventions/01-naming-conventions.md` (Rule 7.1 v2), `02-schema-design.md` (Rule 13), `02-spec/19-main-worker-service/11-split-db-tier-reconciliation.md` (referenced).
...
Every error MUST include Code Red fields: `Path`, `Reason`, `Operation`, `Module` (per `02-spec/03-error-manage/`).

## 6. Concurrency guardrails

- Worker CLI is the sole writer to Session DBs it owns during a capture run. Processing CLI opens the same Session DB only after Worker's `sessions.completed` IPC (`02-spec/21-app/76-cli-log-and-ipc.md`) or via `--force` with `busy_timeout` raised to 15000.
...
**Sources:** `02-spec/06-seedable-config-architecture/{00,01,02}` and `02-spec/21-app/76-cli-log-and-ipc.md` §Seedable config (line 128-130).
...
Cross-ref: gap `.ai-memory/issues/40-seedable-config-format-split.md` documents that specs 06 and 76 do not explicitly reconcile the JSON/TOML split; this cheatsheet is the operative reconciliation until either spec is updated.

## 9. DB conventions binding for CLI migrations (Plan 90 Step 4)

**Sources:** `02-spec/04-database-conventions/{00-overview.md,01-naming-conventions.md,02-schema-design.md,03-orm-and-views.md,07-split-db-pattern.md}`.
...
- Errors emitted to stdout use the same envelope; the JSONL log record from `02-spec/21-app/76-cli-log-and-ipc.md` is a separate artifact and must NOT replace or shadow the stdout envelope.
- `76` §JSONL PascalCase requirement is consistent with envelope casing; the `74/76` snake-vs-PascalCase contradiction filed in `.ai-memory/issues/39-spec-74-77-inconsistencies.md` stays open and must be resolved to PascalCase across all four specs.

---

## 11. Generic CLI binding rules (Step 6 finding)

Sources read: `02-spec/13-generic-cli/{03,04,06,07,09,16,17,20}.md`. Spec is written in Go idioms but rules are language-agnostic and BIND both Python CLIs.
...
Language mismatch: generic CLI spec is Go. Python translations (argparse for flag, importlib.resources for embed, sys.stderr for stderr) are permitted verbatim analogs. Do NOT introduce Click or Typer; stdlib `argparse` only, per `.ai-memory/coding-guidelines.md` "minimum surface" rule.

---

## 12. PowerShell wrapper rules (Step 7 finding)

Sources read: `02-spec/11-powershell-integration/{00-overview,01-configuration-schema,02-script-reference,03-integration-guide,04-error-codes}.md`. Spec targets a Go+React `run.ps1` runner and does NOT natively cover Python CLI wrappers, so a scoped adaptation is required for Plan 90 wrappers (Steps 40, 77-84).
...
- Reserved wrapper-only exit codes: `9510` = Python not found, `9511` = venv activation failed, `9512` = CLI module import failed. Register these alongside the existing 9500-9509 in `02-spec/11-powershell-integration/04-error-codes.md` at Step 84 (spec-edit step; not this turn).
...
- Installer script `install.ps1` (Step 87-90) is a SEPARATE artifact governed by `02-spec/21-app/77-cli-powershell-and-release.md`; it downloads release assets, verifies checksum, unpacks, and drops wrappers into `%LOCALAPPDATA%\vision-app\bin\` + adds to PATH. Not the same as the per-command wrappers here.

Conflict note: `.ai-memory/coding-guidelines.md` (folder-level) wins over `.ai-memory/coding-guidelines.md` (flat) per prompt rules. Neither addresses PowerShell directly, so the 11-powershell-integration spec is authoritative for PS style.
