...
Source: `02-spec/02-coding-guidelines/` @ v3.2.0 (2026-04-16). Captures the meta
...
- **CODE RED priorities** (all mandatory, block merge):
  1. Error management is #1 — wrap business logic in error handling from line 1.
     Follow `02-spec/03-error-manage/` (read separately).
...
Primary stack: TypeScript + React + TanStack Start (see `02-spec/21-app/`).
...
- Error management (`02-spec/03-error-manage/`) — read in step 8-9.

## Open follow-ups (feed later plan steps)

- Read `02-spec/02-coding-guidelines/08-file-folder-naming/` and `11-security/`
  (Plan 39 step 6).
- Read language folders 03/04/05/07/09/10 headers only (step 7).
- Full `02-spec/03-error-manage/` read (steps 8-9).
