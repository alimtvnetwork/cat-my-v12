...
Source of truth: `02-spec/21-app/53-ui-seed-facade.md` (Section 0 v2 Addendum), `02-spec/21-app/53-ui-improvements-v4.md` section 9, `02-spec/21-app/52-sdk-facade-pattern.md`.
Plans: `.ai-memory/plans/pending/79-ui-improvements-v4.md` (steps 5-7, 13-19, 47) and `.ai-memory/plans/pending/86-ui-v4-json-seed-facade-completion.md` (frozen v2 pipeline).
...
- Errors: every facade failure routes through the 3-tier error funnel (`02-spec/03-error-manage`); no silent catch.
...
4. Every fake / IndexedDB-backed facade gets a matching TODO file under `.ai-memory/pending-facades/NN-<domain>-facade.md`. The TODO names: current fake behavior, target real-SDK call, migration checklist, and owner. Removing a facade fake without deleting or completing its TODO is a lint failure.
...
6. Errors from any facade go through the existing `errorStore` with a `correlationId` (already added in Plan 71); silent catch or `try/catch` that swallows is a violation of `02-spec/03-error-manage`.
...
Every `.ai-memory/pending-facades/NN-<domain>-facade.md` follows:
...
- Every new facade PR must add or update its `.ai-memory/pending-facades/NN-*.md` in the same commit.
