...
| `phpcs/`         | `coding-guidelines-ruleset.xml`                             | PHP      | PHP_CodeSniffer ruleset mirroring `02-spec/02-coding-guidelines/` |
...
- Treat these files as **generated artifacts** of `02-spec/02-coding-guidelines/` and `02-spec/17-consolidated-guidelines/02-coding-guidelines.md`. Never edit a ruleset in isolation — update the spec first, then regenerate/mirror.
- The authoring guide for these files is `02-spec/17-consolidated-guidelines/27-linter-authoring-guide.md`.
...
- Linter execution scripts (Python/shell wrappers) live in the sibling `linter-scripts/` folder — see `.ai-memory/memory/05c-linter-scripts.md` (pending Step 7).
