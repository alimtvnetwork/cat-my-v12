...
- Total markdown files in `02-spec/`: 879 files across 26 major modules (`01-spec-authoring-guide` through `25-app-audit`).
- Total markdown files in `.ai-memory/`: 697 files across memory, plans, prompts, issues, and spec commands.
- Total memory files in `.ai-memory/memory/`: 134 markdown files.
- Consolidated guidelines in `02-spec/17-consolidated-guidelines/`: 33 policy documents.
- Spec authoring files in `02-spec/01-spec-authoring-guide/`: 17 guideline documents.
- Issues tracked in `.ai-memory/issues/`: 35 issue tracking records.
- CI/CD issues in `.ai-memory/cicd-issues/`: 0 files (folder absent on disk, CI remediation tracked in pending plan).
- Open ambiguities in `.ai-memory/ambiguity-questions/01-new-ambiguity/`: 0 blocking ambiguities.
- Resolved ambiguities in `.ai-memory/ambiguity-questions/02-ambiguity-resolved/`: 1 resolved record (`01-code-need-to-follow-guidelines.md`).
- Active pending plan in `.ai-memory/plans/pending/`: 1 plan (`02-cicd-remediation.md`) with 4 active subtasks (`SS-01` to `SS-04`).
