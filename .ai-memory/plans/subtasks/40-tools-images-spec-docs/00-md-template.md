...
Every `02-spec/24-app-ui-design-system/tools-images/NN-<slug>.md` file MUST follow this shape. The audience is an AI reading the MD without access to the image, so be exhaustive: transcribe visible text verbatim, name every color with role, describe every control and its expected behavior.
...
What a downstream AI/agent should infer or NOT infer from this screen: mappings to `02-spec/21-app` primitives (rule kinds, ROI shapes, tool families), to `EditorRuleKind` enum values, to menu-group ids. Call out any KEYENCE-specific terminology and its equivalent in this project.
```
