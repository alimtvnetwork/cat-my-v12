...
Scope of spec read: every `.md` under `02-spec/03-error-manage/{01-error-resolution,02-error-architecture,03-error-code-registry}` (110+ files). Below is the compliance gap between the spec and the shipped app.
...
- Floating notices must never be half-clipped. Source: our own command `.ai-memory/02-spec/commands/25-hide-clipped-floating-notices.md`.
