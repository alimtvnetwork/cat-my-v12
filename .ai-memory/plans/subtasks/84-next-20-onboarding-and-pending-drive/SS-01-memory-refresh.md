...
listed in `.ai-memory/memory/index.md`. Key active constraints:
...
- Code Red caps in `.ai-memory/memory/01-code-red.md`; error architecture
...
- v2 execution order locked at `02-spec/21-app/62-v2-execution-order.md`.
- Blind-AI signoff at `02-spec/25-app-audit/99-signoff.md` (mean 98.0/100,
...
`02-spec/` root folders scanned; deep-dive deferred to Plan 84 Step 2.

- `01-general/` — product vision, glossary.
- `02-coding-guidelines/` — canonical coding rules (source of truth
  for `.ai-memory/memory/24-coding-and-error-rulebook.md`).
...
### `02-spec/02-coding-guidelines/` (v3.2.0, 2026-04-16)
...
### `02-spec/21-app/` (Vision Inspection app)
...
- `40-error-manage.md`, `41-logging.md`, `42-observability.md`, `44-security-privacy.md` — the app-level projections of `02-spec/03-error-manage/` that Plan 83 must honor when wiring showToastError/captureException on new failure edges.
...
No contradictions between `02-spec/` current text and the loaded memory. Nothing to escalate. Steps 3-20 of Plan 84 proceed without spec-side blockers.
...
Deltas vs prior onboarding: editor canvas (SelectionOverlay + CanvasViewport) and setup.rules remain the heaviest surfaces, directly matching Plan 83 (editor bridges, seed hardening) and Plan 82 (UI V4) scope. `__root.tsx` at 25KB is high for a root layout, consistent with `.ai-memory/issues/31-duplicate-breadcrumb.md` risk area (single-mount breadcrumb constraint).
