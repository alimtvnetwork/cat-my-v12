...
Run `grep -r "BE/routes" spec/` and `.ai-memory/`. For each hit, update the
...
If `.ai-memory/overview.md` has an architecture diagram referencing `BE/routes/`,
...
- `grep -r "BE/routes" spec/ .ai-memory/` returns zero results.
