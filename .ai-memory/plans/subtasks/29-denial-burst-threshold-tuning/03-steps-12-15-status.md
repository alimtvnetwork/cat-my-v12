...
Date: 2026-07-16
Source plan: `.ai-memory/plans/pending/29-denial-burst-threshold-tuning.md` (repathed this turn)
...
- SH-Q-07 (sample-size question) in `02-spec/21-app/shell/24-open-questions.md` remains OPEN. Explicit non-closure.
...
1. Land an anonymised 90-day export at `.ai-memory/plans/subtasks/29-denial-burst-threshold-tuning/evidence/90d.jsonl` (git-ignored).
...
4. Only then update `02-spec/21-app/69-v2-denial-tuning-contract.md §4` and `02-spec/03-error-manage/03-error-code-registry/*` with any newly required codes.
