...
Date: 2026-07-14
Source plan: `.ai-memory/plans/pending/29-denial-burst-threshold-tuning.md`

## Correction to plan assumptions

The plan header assumed `02-spec/22-security/`, `app/core/security/denial_burst.py`, and `tests/security/`. None of these exist. The actual layout is:

- Spec home: `02-spec/21-app/69-v2-denial-tuning-contract.md` (v2.0.3 locked contract, Plan 19 Step 2).
...
- Canonical defaults: `app/core/config/settings_store.py:45-48` (per `02-spec/21-app/69-v2-denial-tuning-contract.md:31`).
- Section schema: `02-spec/21-app/69-v2-denial-tuning-contract.md:20-33` locks the two-key payload shape and the invalid-type error `E_CFG_INVALID_SECURITY`.
...
- `02-spec/03-error-manage/03-error-code-registry/*` owns the code catalog. Rank 4 must register any new codes (candidate: `W_SEC_BURST_APPROACHING`, `I_SEC_BURST_THRESHOLDS_LOADED`) before use.
- Missing-log-is-a-bug rule per `02-spec/21-app/shell/11-observability.md` applies.

## Step 6 - Consequences for the plan

- Rename the plan's spec target from `02-spec/22-security/threshold-tuning.md` to `02-spec/21-app/69-v2-denial-tuning-contract.md` §4 amendment + new `02-spec/21-app/69a-v2-denial-tuning-evidence.md` for the numeric evidence.
...
- SH-Q-07 in `02-spec/21-app/shell/24-open-questions.md` (Rank 4 telemetry sample size) still open. Data phase must resolve it.
