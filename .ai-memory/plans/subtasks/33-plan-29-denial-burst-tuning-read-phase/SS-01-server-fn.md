...
## Error rules (per `.ai-memory/memory/03-error-manage.md`)
...
BLOCKER surfaced in `.ai-memory/memory/v2/plan29/10-telemetry-inventory.md` §"Data-source blocker": the audit sink lives in the Python worker (SQLite `audit_log`), not Supabase. A TanStack server fn using `context.supabase` has nothing to query.
