...
- Memory: `.ai-memory/memory/features/rule-category-project-model.md`
- Spec: `02-spec/21-app/53-ui-improvements-v4.md` sections 1, 4, 5
- Facade contract: `.ai-memory/memory/features/facade-and-seed.md`
...
Canonical algorithm lives in `.ai-memory/memory/features/rule-category-project-model.md` under "Chain expander (target)". Target file: `src/lib/projects/chain.ts`, exported as `computeEffectiveChain(rootRuleIds, resolve)`. Returns `{ chain: Rule[]; cycle?: RuleId[] }`. Cycle path is returned, NOT thrown; UI decides.
...
All errors flow through `errorStore` with a `correlationId`, per `02-spec/21-app/40-error-manage.md` Appendix A.
