...
Rewrite `02-spec/24-app-ui-design-system/09-UI-improvements-v2.md` (the V2 UI backlog the user filed originally) as a single consolidated status file: what is DONE, what is PENDING, what is AMBIGUOUS. The consolidated file lives at the END of the spec-24 sequence (highest suffix so far is `99-consistency-report.md`) and its filename ends in `-v2-enhancement` so `rg v2` finds it every time. Primary inputs:

- `02-spec/24-app-ui-design-system/09-UI-improvements-v2.md` — the original V2 backlog.
- `.ai-memory/plans/done/66-ui-v3-missing-completion.md` — first execution wave.
- `.ai-memory/plans/done/67-ui-fluid-modern-v2-v3-completion.md` — 50-step execution wave that closed most V2 items.
...
4. Decide the target filename + sequence for the consolidated file. Highest suffix in `02-spec/24-app-ui-design-system/` today is `99-consistency-report.md`; the user wants "the next one, at the end, findable by V2". Target: `02-spec/24-app-ui-design-system/99d-ui-improvements-v2-enhancement.md`. See ./subtasks/68-ui-improvements-v2-enhancement/SS-04-target-filename-decision.md.
...
7. Fill the `Pending` section: each item links to its owning pending plan under `.ai-memory/plans/pending/` (e.g. 29, 32, 35, 36, 41, 43-slice-x); no orphan items.
...
9. Wire the new file into the spec index: add a one-line pointer at the top of `02-spec/24-app-ui-design-system/09-UI-improvements-v2.md` ("SUPERSEDED — see 99d-ui-improvements-v2-enhancement.md"), add an entry in `02-spec/24-app-ui-design-system/00-overview.md`, and log the addition in `02-spec/24-app-ui-design-system/98-changelog.md`.
10. Verify + release: `rg -l 'UI improvements v2'` returns the new file, `bunx tsgo --noEmit` clean (no code changed but confirms nothing regressed), bump minor version, add CHANGELOG + RELEASE_NOTES entries, update README pinned version. Then move THIS plan file from `.ai-memory/plans/pending/68-*.md` to `.ai-memory/plans/done/68-*.md` and flip `Status:` to `completed` in the same move.
...
- Steps 5-8: `rg -n '^##' 02-spec/24-app-ui-design-system/99d-ui-improvements-v2-enhancement.md` shows the four canonical sections; every listed item under `Pending` resolves to a real file in `.ai-memory/plans/pending/`.
- Step 9: `rg -n '99d-ui-improvements-v2-enhancement' 02-spec/24-app-ui-design-system` returns the pointer + overview + changelog hits.
- Step 10: `bunx tsgo --noEmit` clean; new version tag present in `package.json`, `CHANGELOG.md`, `RELEASE_NOTES.md`, `README.md`; this plan file lives under `.ai-memory/plans/done/` with `Status: completed`.

## Appended from prior pending tasks

None. This plan is scoped strictly to consolidating the V2 backlog status; the sixteen existing pending plans (29, 32, 35, 36, 38, 39, 40, 41, 44, 46, 49, 50, 51, 52, 57, 58, 59, 61, 62, 63) remain in `.ai-memory/plans/pending/` and are only referenced from step 7 for cross-linking.
