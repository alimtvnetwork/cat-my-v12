...
Files involved: `.ai-memory/memory/audit/`, `readme.md`, `changelog.md`, `release_notes.md`, `.ai-memory/plans/pending/{02,05,07,08}-*.md`, `app/capture/perf_harness.py`, `app/core/security/{consent,tokens}.py`, `src/lib/ids/ulid.ts`, `src/routes/**`, `tests/**`.

Related captured guidance: `.ai-memory/02-spec/commands/01-plan-50-workflow.md`, `.ai-memory/02-spec/commands/02-ip-guardrail.md`.
...
8. Run full test matrix (`pytest`, `vitest`, Playwright, axe) and record pass counts + p50/p95/p99 fps into `.ai-memory/memory/audit/evidence/v0.105.0/`.
9. Execute Audit Rescore #3 into `.ai-memory/memory/audit/99-audit-report.v0.105.0.md`, crediting v0.102–v0.105 evidence (tokens, healthz/ready, perf harness, consent, E2E, a11y). Expected mean 83–86.
...
12. Archive completed plans: `mv` `.ai-memory/plans/pending/{02,05,07,08}-*.md` → `.ai-memory/plans/done/` (existing folder), flipping each `Status:` frontmatter `pending → completed` in the same move.
...
14. Reconcile `.ai-memory/plans/completed/` vs existing `.ai-memory/plans/done/`: keep `done/`, delete empty `completed/` if created, and update `.ai-memory/02-spec/commands/01-plan-50-workflow.md` to match. Prevents future lifecycle drift.
15. Register `01-prompts/106-next-task.md` describing post-v1 backlog (hardware bridge stub, Lovable Cloud auth surface, real capture driver) and close this plan by moving it to `.ai-memory/plans/done/09-audit-closeout-v1-tag.md` with `Status: completed`.
...
- Step 12: `ls .ai-memory/plans/pending/` returns empty; each archived file's frontmatter says `Status: completed`.
- Steps 13–15: `mem://index.md` diff shows v1 marker; prompt 106 file exists; this plan file lives at `.ai-memory/plans/done/09-audit-closeout-v1-tag.md`.
