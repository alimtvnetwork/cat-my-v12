# Audit Consolidation into 02-spec/25-app-audit
...
Prior audit artifacts were processed into `02-spec/25-app-audit/latest/` and then deleted per the user's single-source instruction. This plan consolidates the scattered audit outputs into one canonical current audit bundle with current contracts and repaired cross-links.

Related:

- `.ai-memory/02-spec/commands/01-plan-50-workflow.md`
- `.ai-memory/02-spec/commands/04-spec-vs-code-reaudit.md`
- `.ai-memory/02-spec/commands/05-spec-vs-code-audit-30step.md`
- `.ai-memory/02-spec/commands/06-spec-vs-code-audit-15step.md`
- Prior pending: `16-rule-bundle-import-export-spec.md`, `17-v2.0.2-vendor-sdk.md` (unrelated tracks, left in place)

## Steps

1. Inventory every file under `02-spec/25-app-audit/` (top-level + memory/ + evidence/ + version subfolders) into `./subtasks/18-audit-consolidation/ss-01-inventory.md`.
...
3. Define the target folder shape: `02-spec/25-app-audit/{00-overview.md, 00-rubric.md, 00-scope.md, CONVENTIONS.md, latest/}`. Document in `./subtasks/18-audit-consolidation/ss-03-target-shape.md`.
4. Create `02-spec/25-app-audit/latest/`.
5. Identify the canonical latest audit (v1.42.1-full signoff, mean 95.5/100, 0 findings).
6. Move `02-spec/25-app-audit/latest/*` contents into `02-spec/25-app-audit/latest/`.
...
14. Rewrite `02-spec/25-app-audit/00-overview.md` to describe the new `latest/` single-source shape and how to read the audit trail.
...
24. Create `02-spec/25-app-audit/latest/readme.md` describing the current audit status (version, score, finding count, signoff date).
25. Generate a single consolidated `02-spec/25-app-audit/00-history-timeline.md` as a minimal lineage summary for the current audit.
26. Grep the current docs for removed archive references and rewrite to current paths.
27. Grep for `.ai-memory/memory/audit/` references (should be zero after prior turn) and rewrite any stragglers.
...
31. Update `readme.md` audit section to point at `02-spec/25-app-audit/latest/` as the canonical current-state entry.
32. Update `changelog.md` if it links audit reports to use new paths.
33. Update `RELEASE_NOTES` files for the same.
34. Update `.ai-memory/memory/index.md` Core section audit line to point at `02-spec/25-app-audit/latest/`.
35. Update `.ai-memory/memory/index.md` Memories section entry for the v1.0.0 signoff link.
36. Update `02-spec/spec-index.md` if it lists audit files.
37. Update `02-spec/21-app/*` any files linking to audit paths.
38. Update `02-spec/22-app-issues/*` overview if it references audit outputs (it should NOT, since issues are separate now).
39. Verify no file inside `02-spec/25-app-audit/` links to a sibling that has moved without update. See `./subtasks/18-audit-consolidation/ss-06-crosslink-repair.md`.
40. Add a `02-spec/25-app-audit/CONVENTIONS.md` documenting: every new audit run refreshes `latest/`, folds useful old conclusions forward, and deletes processed old files.
...
48. Update `.ai-memory/memory/index.md` `Updated:` timestamp.
49. Bump project version per convention (patch bump) and update README version banner.
50. Move this plan file to `.ai-memory/plans/done/18-audit-consolidation.md` and flip `Status: pending` to `Status: completed`.

## Verification

- `02-spec/25-app-audit/` matches the single-source target shape from Step 3.
...
- `02-spec/25-app-audit/latest/` contains the v1.42.1-full current bundle and signoff.
- Processed archive folders are absent.
- `readme.md` and `.ai-memory/memory/index.md` link to `02-spec/25-app-audit/latest/` and resolve.
