...
Two-track improvement: (1) enforce the coding guideline and error-manage spec across `src/**` with no logic changes (PascalCase, no magic strings, boolean/flow readability, shared constants), and (2) ship a UI error dialog gated by a new `AppMode` flag (`Dev` default, `Test`, `Prod`) selectable at publish time. Spec docs under `02-spec/02-coding-guidelines/**` and `02-spec/03-error-manage/**` are updated to match every rename.

Captured commands:

- `.ai-memory/02-spec/commands/19-error-dialog-dev-mode.md`
- `.ai-memory/02-spec/commands/20-pascalcase-no-magic-strings.md`
- `.ai-memory/02-spec/commands/21-code-quality-boolean-and-flow.md`
...
1. Read every file in `.ai-memory/coding-guidelines.md` and log delta vs current `src/`.
2. Read every file in `02-spec/02-coding-guidelines/**` and record binding rules.
3. Read every file in `02-spec/03-error-manage/**` including `02-error-architecture/04-error-modal/*`.
4. Read `.ai-memory/memory/02-naming.md`, `03-error-manage.md`, `22-coding-guidelines-digest.md`, `24-coding-and-error-rulebook.md`.
5. Produce a written rulebook digest at `.ai-memory/memory/25-plan43-digest.md` (not a mirror, canonical only if new rules emerge).
...
20. Compile the audit findings into `.ai-memory/plans/subtasks/43-coding-quality-error-dialog-and-mode-flag/audit-findings.md`.
...
26. Create `src/lib/constants/error-codes.ts` mirroring `02-spec/03-error-manage/03-error-code-registry`.
...
66. Sync `02-spec/02-coding-guidelines/02-typescript/*` with the boolean, magic-string, and PascalCase rules. See ./subtasks/43-coding-quality-error-dialog-and-mode-flag/SS-05-pascalcase-rename-and-spec-sync.md.
67. Sync `02-spec/02-coding-guidelines/08-file-folder-naming/*` with file naming rules used in the audit.
68. Sync `02-spec/03-error-manage/02-error-architecture/04-error-modal/02-react-components.md` with the new provider.
69. Sync `02-spec/03-error-manage/02-error-architecture/04-error-modal/00-overview.md` with mode gating.
70. Add `02-spec/03-error-manage/02-error-architecture/04-error-modal/07-mode-gating.md` describing `Dev`/`Test`/`Prod`.
71. Add cross-links from spec `21-app/61-sample-images-and-focus.md` to `CameraPov` and `SampleId` constants.
72. Update `02-spec/24-app-ui-design-system/03-canvas.md` where identifiers changed.
73. Update `.ai-memory/memory/02-naming.md` with the finalized PascalCase rules.
74. Update `.ai-memory/memory/03-error-manage.md` with the dialog + mode gating.
75. Update `.ai-memory/memory/index.md` to reference the new memory entries.
...
88. Sweep remaining `.ai-memory/issues/` entries touched by this plan and mark resolved ones.
89. Cross-check every completed step against `02-spec/02-coding-guidelines/97-acceptance-criteria.md`.
90. Cross-check every completed step against `02-spec/03-error-manage/97-acceptance-criteria.md`.
...
93. Update `.ai-memory/02-spec/commands/19-error-dialog-dev-mode.md` status once shipped.
94. Update `.ai-memory/02-spec/commands/20-pascalcase-no-magic-strings.md` status once shipped.
95. Update `.ai-memory/02-spec/commands/21-code-quality-boolean-and-flow.md` status once shipped.
96. Move each completed subtask file to `.ai-memory/plans/subtasks/43-coding-quality-error-dialog-and-mode-flag/completed/` (or flip `Status:` in place).
97. Flip this plan's `Status:` frontmatter to `completed`.
98. `mv .ai-memory/plans/pending/43-coding-quality-error-dialog-and-mode-flag.md .ai-memory/plans/completed/43-coding-quality-error-dialog-and-mode-flag.md` (creating `completed/` if missing).
...
Pulled from `.ai-memory/plans/pending/` and left untouched by this plan; they remain owned by their own files and must not be re-scoped here:
