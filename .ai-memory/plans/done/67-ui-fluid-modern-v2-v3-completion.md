...
User asked to check `02-spec/24-app-ui-design-system/09-UI-improvements-v2.md`
...
- `02-spec/24-app-ui-design-system/09-UI-improvements-v2.md` (history, kept)
- `02-spec/24-app-ui-design-system/10-UI-improvements-v3.md` (source of truth)
- `.ai-memory/plans/pending/66-ui-v3-missing-completion.md` (parent stream; this plan is the fluid-polish + connect-flow slice of it, does not replace it)
- `.ai-memory/issues/09-setup-ui-not-modern.md`
- `.ai-memory/issues/16-project-section-create-flow-broken.md`
- `.ai-memory/issues/21-panels-not-draggable-floatable.md`
- `.ai-memory/issues/22-duplicate-header-still-present.md`

Execution rule: one AI turn = one step. Each step ends with typecheck +
lint + unit + relevant e2e green per `.ai-memory/02-spec/commands/24-cicd-lint-integration.md`,
...
1. Baseline: run tsgo, lint, vitest, playwright_home, playwright_smoke; record in `.ai-memory/memory/v2/plan67/00-baseline.md`.
...
5. Panel-mobility fix: docked panels keep a visible grip so they can be redragged (closes `.ai-memory/issues/21`).
...
38. Project create flow fix (PR-02): reproduce `.ai-memory/issues/16`, patch dead ends, regression test.
...
49. Update v3 status matrix in `02-spec/24-app-ui-design-system/10-UI-improvements-v3.md`; flip every row this plan closed to DONE.
50. Closeout: bump `package.json` minor, update `CHANGELOG.md`, `RELEASE_NOTES.md`, `README.md` pin; `mv` this file to `.ai-memory/plans/done/67-ui-fluid-modern-v2-v3-completion.md` and flip `Status:` to `done`.
...
Rows still open in `.ai-memory/plans/pending/` that this plan touches directly:
...
If any row in `.ai-memory/ambiguity-questions/02-ui-v3-open-questions.md` is
