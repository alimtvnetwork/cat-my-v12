...
Direction: spec first (`02-spec/21-app/40..43`), then schema + UI, then
Python contract. No behavior change until each layer lands.

Captured input:

- `.ai-memory/02-spec/commands/18-rule-condition-and-validation-order.md`

Applicable guidelines (all present):

- `.ai-memory/coding-guidelines.md`
- `02-spec/02-coding-guidelines/**` (enums per concept, no magic strings)
- `02-spec/03-error-manage/**` (AppError + ErrorCode required)

Primary files touched: `02-spec/21-app/40..43*.md` (new),
...
1. Read `02-spec/21-app/16-processing-pipeline.md`,
   `02-spec/21-app/17-parallelism-guarantees.md`, and the current
   `src/lib/editor/schema.ts` end-to-end; list every field that must
   evolve.
2. Draft `02-spec/21-app/40-rule-condition-model.md` (see
   `./subtasks/42-rule-conditions-and-validation-order/SS-01-spec-authoring.md`).
3. Draft `02-spec/21-app/41-color-condition.md` with Current / Dense2 /
   Dense3 / Picked param shapes (see SS-01).
4. Draft `02-spec/21-app/42-validation-order.md` covering Parallel vs
   Sequential semantics + short-circuit rules (see SS-01).
5. Draft `02-spec/21-app/43-rule-controller-ui.md` with the per-rule and
   ruleset UI contract (see SS-01).
6. Append the four new spec files to
   `02-spec/21-app/99-consistency-report.md` and update
   `02-spec/spec-index.md`.
...
29. Update `.ai-memory/coding-guidelines.md` to
...
- Spec: files exist at `02-spec/21-app/40..43*.md` and are linked from
  `02-spec/spec-index.md`.
...
- Manual: preview shows the new controller panel, segmented match
  ...
  Sequential prefixes.
- Guideline: `.ai-memory/coding-guidelines.md`
  references the new enum modules.

## Appended from prior pending tasks

Scan of `.ai-memory/plans/pending/` at plan creation time. These
