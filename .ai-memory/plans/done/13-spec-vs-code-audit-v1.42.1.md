...
- Command: `.ai-memory/02-spec/commands/06-spec-vs-code-audit-15step.md`
- Rubric: `02-spec/25-app-audit/00-rubric.md`
- Prior audits: processed into `02-spec/25-app-audit/latest/`.
...
10. Author area report `22-ui-and-server-fns.md` (routes, HMI components, `src/lib/*.functions.ts`, `src/lib/*.server.ts`, run-store) and specifically verify `02-spec/21-app/66-v2-vendor-discovery.md` Proving tests still names the 4 E2E steps present in `tests/e2e/ops_vendor_smoke.py`.
...
15. Flip `Status:` to `completed` and `mv .ai-memory/plans/pending/13-spec-vs-code-audit-v1.42.1.md .ai-memory/plans/done/`.
...
- `02-spec/25-app-audit/latest/` links every new file in the current bundle.
- No file under `app/**` or `src/**` modified (audit-only).
- Plan file lives in `.ai-memory/plans/done/` at end; not duplicated in `pending/`.

## Appended from prior pending tasks

- `.ai-memory/plans/pending/12-spec-vs-code-audit-v1.28.md` — 30-step audit at v1.28.0 snapshot. Superseded in scope by this v1.42.1 audit; keep pending for its own historical snapshot or close via a follow-up plan.
