...
**Predecessors:** Plan 04 (spec, done). Consistency report `02-spec/22-app-issues/consistency-0.69.0.md` green.
**Parks:** Plan 02 (Control Automation UI redesign) — remains parked until this plan reaches M4.

## 1. Goal

Move from spec to running v1. Every milestone binds directly to a locked spec section and one or more acceptance gates in `02-spec/21-app/97-acceptance-criteria.md` (A-01…A-23). No milestone invents a contract — it implements one.
...
- Refresh `.ai-memory/memory/08-vision-inspection-app.md` to include Q-07 (single operator), Q-08 (content-addressed refs), Q-09 (dual-clock), Q-10 (health token) rules.
- Snapshot the `01-prompts/` directory state and archive Plan 04 links out of pending views.
- **Acceptance signal:** consistency report 3.3 flips from PENDING to PASS.

### M1 — Root DB + migrations skeleton (2–3 days)

- Implement `backend/db/root.db` schema per `02-spec/21-app/21-root-db.md` and migration runner per `26-migrations.md`.
...
- **Status (v0.76.0):** Complete. Artifacts: `app/core/errors/{__init__,codes,types}.py`, `src/components/BugErrorModal.tsx`, `src/routes/__root.tsx` (modal mount), `02-spec/22-app-issues/consistency-0.76.0.md` (7/7 PASS).
...
2. `02-spec/22-app-issues/consistency-<version>.md` reports 7/7 PASS (no PENDING).
3. All 23 acceptance gates in 97 flip to PRESENT with cited evidence.
4. Plan is moved from `.ai-memory/plans/pending/` to `.ai-memory/plans/done/`, and Plan 02 is either merged into scope or explicitly re-parked with a reason.
