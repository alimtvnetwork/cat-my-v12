...
Restore the home-first workflow UI, remove the `src_v3` rollback direction, preserve an always-available Home route, and improve the React app menu into a Dexter-style HMI command surface using the attached screenshots as visual references. Captured command: `.ai-memory/02-spec/commands/12-home-dexter-ui-flow.md`; captured issues: `.ai-memory/issues/13-home-screen-regression.md`, `.ai-memory/issues/14-src-v3-rollback-regression.md`, `.ai-memory/issues/15-global-home-menu-missing.md`.

Conflict noted: the pasted generic lifecycle text says `completed/`, but the active project lifecycle command says archived plans go to `.ai-memory/plans/done/`; this plan follows `.ai-memory/02-spec/commands/08-plan-lifecycle-done-folder.md`.
...
12. Close by updating `.ai-memory/memory/index.md` with the landed home/menu repair evidence and moving this plan to `.ai-memory/plans/done/37-home-dexter-ui-repair.md` with `Status: completed`.
