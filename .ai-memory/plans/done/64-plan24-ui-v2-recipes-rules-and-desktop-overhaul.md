...
Author the full spec revision and UI overhaul described in `02-spec/24-app-ui-design-system/09-UI-improvements-v2.md`. Rename the "Recipe" concept to "Rule Set / Rules", restructure Setup (Camera / Rules / Lighting), compact the global header with breadcrumb + Back/Forward, add a Google-Meet-style dockable running-process pill, rebuild the Rules editor (Photoshop-style Layers/Preview/Tools panels, custom shape design mode, JS user-functions, Flaw Detection, Barcode/QR, blob detection, positional adjust), fix the broken Project section, and add per-rule / per-rule-set / per-project export-import (JSON, YAML, zipped SQLite).

Captured command: `.ai-memory/02-spec/commands/22-ui-v2-recipes-rules-and-100-step-plan.md`.
Captured issues:

- `.ai-memory/issues/16-project-section-create-flow-broken.md`
- `.ai-memory/issues/17-menu-hover-jitter-and-padding.md`
- `.ai-memory/issues/18-header-duplicated-control-automation.md`
- `.ai-memory/issues/19-rules-editor-program-panel-and-layer-arrow.md`

Open ambiguities: `.ai-memory/ambiguity-questions/01-ui-v2-open-questions.md` (Q1-Q21). Steps below that depend on these are annotated `(blocked by Qn)`; the executor should ask for the answer before running that step.
...
1. Create the assets folder `02-spec/24-app-ui-design-system/assets/` with a `README.md` placeholder listing image slots referenced by 09-UI-improvements-v2.md; user will drop PNGs later.
2. Add `.ai-memory/ambiguity-questions/` to `.ai-memory/what-to-read.md` so future AIs know to consult it first.
3. Freeze vocabulary in `02-spec/24-app-ui-design-system/00-overview.md`: `Rule Set` (aggregate), `Rule` (leaf), `Category`, `Project`. Remove "Recipe" from spec after Q1 is confirmed. (blocked by Q1)
...
5. Create `02-spec/24-app-ui-design-system/10-navigation-shell.md` describing the new compact header (breadcrumb + Back/Forward + running pill slot) and Setup entry with three tiles.
6. Create `02-spec/24-app-ui-design-system/11-running-process-pill.md` (Google-Meet pattern: draggable, dockable, stop-from-pill, click-to-jump).
7. Create `02-spec/24-app-ui-design-system/12-rules-editor-shell.md` covering Photoshop-style Layers/Preview/Tools palettes, dock/float persistence, full-width layer row with right-side chevron.
8. Create `02-spec/24-app-ui-design-system/13-rule-kinds-catalogue.md` enumerating: Rectangle OCR, Circular OCR, Custom Shape OCR, Presence, Absence, Flaw Detection, Barcode QR, Blob Detection, Positional Adjust (edge width / edge pitch), User JS Function. One page per kind.
9. Create `02-spec/24-app-ui-design-system/14-design-mode-custom-shapes.md` describing the design-mode overlay, SVG compile output, import/export of shapes as reusable assets.
10. Create `02-spec/24-app-ui-design-system/15-export-import.md`: JSON + YAML + zipped-SQLite for rules and rule sets, project zip layout.

Section B - Detailed spec authoring (steps 11-50)

11. Write `02-spec/24-app-ui-design-system/16-project-lifecycle.md`: create, edit, category selection, rule-set selection with override chain visualisation, Run.
12. Write `02-spec/24-app-ui-design-system/17-camera-setup.md`: fields (FOV, shutter, pockets, gain, resolution) and persistence.
13. Write `02-spec/24-app-ui-design-system/18-lighting-setup.md` scaffold. (blocked by Q4)
14. Write `02-spec/24-app-ui-design-system/19-ai-settings-placeholder.md` explicitly out-of-scope for v1. (blocked by Q20)
15. Write `02-spec/24-app-ui-design-system/20-backend-endpoint-map.md` as a table: UI action -> HTTP verb -> path -> request payload -> response -> side effects.
...
17. Write `02-spec/24-app-ui-design-system/21-filesystem-layout.md`: `data/<rule-set-name>/<rule-set-id>/<rule-id>/{image.<ext>, rules.json, meta.json}` next to the EXE. (blocked by Q12)
18. Add a Mermaid ER diagram at `02-spec/23-app-db/02-rule-sets.mmd` + rendered PNG for the RuleSet + Rule + Category aggregate.
19. Add `02-spec/23-app-db/03-projects.mmd` + PNG covering Project, ProjectRuleSet (join), ProjectCategory, CameraSetting, LightingSetting.
20. Add `02-spec/23-app-db/04-runs-captures.mmd` + PNG for Run, Capture, RuleResult, CaptureAsset.
21. Add `02-spec/23-app-db/05-user-assets.mmd` + PNG for Shape (SVG), JSFunction, ImportedAsset with checksum + provenance.
22. Update `02-spec/23-app-db/00-overview.md` linking the four new diagrams and defining the reference vs snapshot override columns.
23. Add a "SQLite column contract" section in `02-spec/23-app-db/01-root-db-schema.md` enumerating every rule parameter column (shape kind, x, y, w, h, radius, polygon points JSON, threshold, edge_width, edge_pitch, symbology, expected_text, js_fn_id, etc.).
24. Write `02-spec/24-app-ui-design-system/22-override-modes.md` (Reference vs Snapshot semantics with worked examples). (blocked by Q6, Q7)
25. Write `02-spec/24-app-ui-design-system/23-recent-projects-home.md`: Home shows "Recent" chip with dropdown of recently opened projects.
26. Write `02-spec/24-app-ui-design-system/24-categories.md`: create categories, auto-apply rules, per-project category selection.
27. Write `02-spec/24-app-ui-design-system/25-run-flow.md`: pick multiple rule sets, view override chain, drop test images, click Run.
28. Write `02-spec/24-app-ui-design-system/26-validate-single-image.md`: from the rule editor validate the current rule against one uploaded image.
29. Write `02-spec/24-app-ui-design-system/27-user-js-functions.md`: shape, sandbox, allowed globals, import/export as `.js` + metadata. (blocked by Q8)
30. Write `02-spec/24-app-ui-design-system/28-flaw-detection.md`. (blocked by Q9)
31. Write `02-spec/24-app-ui-design-system/29-barcode-qr.md` including symbology list. (blocked by Q10)
32. Write `02-spec/24-app-ui-design-system/30-blob-detection.md`.
33. Write `02-spec/24-app-ui-design-system/31-positional-adjust.md`. (blocked by Q11)
34. Write `02-spec/24-app-ui-design-system/32-export-json-schema.md` with a JSON Schema for a rule and a rule set.
35. Write `02-spec/24-app-ui-design-system/33-export-yaml-schema.md` mirroring the JSON schema. (blocked by Q15)
36. Write `02-spec/24-app-ui-design-system/34-project-zip-layout.md`. (blocked by Q16)
37. Write `02-spec/24-app-ui-design-system/35-import-flow.md`: dry-run diff, conflict resolution (skip/overwrite/rename), signature check.
38. Write `02-spec/24-app-ui-design-system/36-shape-svg-asset.md`: internal SVG format, viewBox, unit, mask semantics.
39. Write `02-spec/24-app-ui-design-system/37-mask-from-image.md`: importing a raster mask, thresholding rules.
40. Write `02-spec/24-app-ui-design-system/38-header-breadcrumb.md`: token map per route -> breadcrumb segments.
41. Write `02-spec/24-app-ui-design-system/39-back-forward.md`. (blocked by Q19)
42. Write `02-spec/24-app-ui-design-system/40-menu-anti-jitter.md`: padding tokens, fixed-box hover rule, allowed animation register.
43. Write `02-spec/24-app-ui-design-system/41-panel-docking-model.md`: dockable Layers/Preview/Tools, per-user layout, LocalStorage vs server persistence.
44. Write `02-spec/24-app-ui-design-system/42-drag-drop-running-pill.md`. (blocked by Q17)
45. Write `02-spec/24-app-ui-design-system/43-rule-editor-toolbar.md`: tool palette, keyboard shortcuts, entering Design Mode.
46. Update `02-spec/24-app-ui-design-system/97b-ui-acceptance-checklist.md` with new checklist rows for every spec above.
47. Update `02-spec/24-app-ui-design-system/98-changelog.md` with an entry pointing to Plan 64.
48. Update `02-spec/24-app-ui-design-system/99-consistency-report.md` to flag any legacy "Recipe" references in code for later cleanup.
...
85. Convert every rule-parameter mutation to route through a `saveRule` server function that writes to **Cloud Postgres** (canonical store per ADR AI-02, `02-spec/21-app/shell/01b-adr-web-vs-desktop-storage.md`). The SQLite bundle format is produced only by the Export flow in steps 86-88; no direct client persistence, no dual-write.
...
94. Add keyboard shortcuts documented in `02-spec/24-app-ui-design-system/43-rule-editor-toolbar.md` (V, R, C, M, T, O, B, F, J).
...
99. Move this plan file from `.ai-memory/plans/pending/` to `.ai-memory/plans/done/` and flip `Status:` to `completed`.
...
- Section A (1-10): `10-` through `15-`, plus `assets/README.md`, `.ai-memory/what-to-read.md` update, vocabulary + naming freeze.
- Section B (11-50): `16-` through `43-`, plus `02-spec/23-app-db/02-` through `05-` diagrams, root DB schema §4, `97b-ui-acceptance-checklist.md` §10, `98-changelog.md` [1.17.0], `99-consistency-report.md` §11, `.ai-memory/ambiguity-questions/00-index-blocked-specs.md`.

### Open ambiguities (must be resolved before Section C)

Q1, Q4, Q6-Q12, Q15-Q17, Q19, Q20 (per `.ai-memory/ambiguity-questions/00-index-blocked-specs.md`).
...
ls 02-spec/24-app-ui-design-system/{10..43}-*.md
# 2. All new DB diagrams exist.
ls 02-spec/23-app-db/{02..05}-*.mmd
# 3. Ambiguity index points at real files.
awk '/spec\// {print $0}' .ai-memory/ambiguity-questions/00-index-blocked-specs.md | \
  grep -oE '02-spec/24-app-ui-design-system/[0-9]+-[a-z-]+\.md' | sort -u | \
