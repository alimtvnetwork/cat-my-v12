...
**Status:** completed · **Seed:** `.ai-memory/memory/audit/91-corrections.md` + `99-audit-report.md` · **Goal:** clear all 14 Blockers so ship status flips from **BLOCKED** → **READY**.
...
- No milestone lands without touching `.ai-memory/memory/audit/03-missing-impl.md` (flip the row) and re-running `10-scores.md` for the affected area.
- Coding guidelines are missing (`.ai-memory/coding-guidelines.md` and `02-spec/coding-guidelines/` both absent). M0 must request them before any Python change lands.

## Milestones

### M0 — Coding guidelines request (blocking)

- **Action:** Ask the user to author `.ai-memory/coding-guidelines.md` (Python conventions, error handling, logging, migration style). Do not proceed to M1 until it exists.
...
- **Files:** rewrite `src/routes/index.tsx` as two-pane; new `src/lib/rpc/{client.ts,guards.ts}`; new server fns `src/lib/jobs.functions.ts` (`listJobs`, `createJob`); update `.ai-memory/memory/index.md` Core (namespace `--ca-*`).
...
- **Files:** new `02-spec/22-app-issues/consistency-0.87.0.md` with §3.1–3.7 (add 3.5 open-questions ledger, 3.7 prompt registry) and §4 gate table using PARTIAL where signals are missing. Freeze v0.76.0 as historical.
- **Signal:** `grep "^### 3\." 02-spec/22-app-issues/consistency-0.87.0.md | wc -l` returns 7; `29-governance.md` rescored ≥ 85.
...
All 14 Blockers flipped RESOLVED in `.ai-memory/memory/audit/index.md`, mean score ≥ 80, consistency-0.87.0.md 7/7 PASS with real evidence, README banner reads **READY**.
