...
4. Write `.ai-memory/memory/v2/plan32/40-closeout.md`: landed slices (1/2/3), full gap-to-commit map, rollback (revert range), and confirmation that `00-pattern-edge-baseline.md` gap list is empty.
5. Move (via `mv`) `.ai-memory/plans/pending/{32,53,54,55}-*.md` to `.ai-memory/plans/completed/` after all tests green; flip each `Status:` to `completed`. Verify `tsgo --noEmit` + `vitest run` (or `pytest`) exit 0 and `git diff --stat` scoped to expected paths.
...
- `ls .ai-memory/plans/pending/` no longer contains 32/53/54/55; `ls .ai-memory/plans/completed/` contains them, each with `Status: completed`.
