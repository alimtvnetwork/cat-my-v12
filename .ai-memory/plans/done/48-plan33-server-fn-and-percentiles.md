...
- `.ai-memory/memory/v2/plan29/20-windows.json` and `.ai-memory/memory/v2/plan29/30-derivation-inputs.md` exist; the memo cites p95/p99 values from `20-windows.json` (grep must find matching numeric literals).
- No threshold constants under `app/` or `02-spec/21-app/40-error-manage.md` were modified (verify with `git diff --stat` on those trees returning zero for constants files).
