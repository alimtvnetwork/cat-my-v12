...
Plan 30 (rule-editor revamp) closed at v3.76.0 with steps 93+ QA gates green, but the pre-93 panel implementation left three gaps flagged as follow-ups in `.ai-memory/memory/index.md` (Plan 30 CLOSED section) and in prior next-task notes:
...
Files most likely involved: `src/components/editor/`, `src/lib/editor/`, `src/routes/setup*.tsx`, `02-spec/24-app-ui-design-system/`, `.ai-memory/memory/04-design-system.md`. No new user commands or issues to capture this turn (none provided).

Related:

- `.ai-memory/plans/done/30-app-ui-rule-editor-revamp.md`
- `.ai-memory/memory/04-design-system.md`
- `02-spec/24-app-ui-design-system/08-testing.md`
...
3. Read `02-spec/24-app-ui-design-system/` in full; extract the panel contract for Lighting, Reference, Number, Color, Pattern-Edge, Blob.
...
10. Wire migration failure path through the project error-management contract (`.ai-memory/memory/03-error-manage.md`): coded log + surfaced user error, no silent catch.
...
15. **DEFERRED (SG-31-01).** PatternEdgePanel removed from this plan. `02-spec/24-app-ui-design-system/05-rule-controller.md` L34-49 has no PatternEdge row. Track a follow-up plan that amends the spec first, then scaffolds the panel.
...
25. Update `02-spec/24-app-ui-design-system/` panel chapter with the six panels' final prop shapes and token bindings.
26. Update `.ai-memory/memory/04-design-system.md` QA evidence table with the extended reports.
27. Update `.ai-memory/memory/index.md` to link plan 31 closure and remove the pre-93 panel gaps follow-up note.
...
29. Move `.ai-memory/plans/pending/31-pre-93-panel-gaps-completion.md` to `.ai-memory/plans/done/` and flip `Status: pending` -> `Status: completed`.
30. Post-close: verify backlog top is now Plan 29 (denial-burst tuning) with no remaining pre-93 gaps referenced anywhere under `.ai-memory/`.

## Verification

- Steps 1-5: subtask files exist under `.ai-memory/plans/subtasks/31-pre-93-panel-gaps-completion/`; gap matrix names every missing panel with file target.
...
- Steps 25-27: `rg "pre-93 panel gaps" .ai-memory/` returns no live follow-up references.
- Steps 28-30: readme, changelog, release_notes pinned to the new minor; `ls .ai-memory/plans/pending/ | grep 31` empty; `ls .ai-memory/plans/done/ | grep 31` present.

## Appended from prior pending tasks

- Plan 29 (`.ai-memory/plans/pending/29-denial-burst-threshold-tuning.md`, Status: pending) is not folded in: it is an independent security-tuning workstream tracked separately with its own subtask notes (`.ai-memory/plans/subtasks/29-denial-burst-threshold-tuning/`). Sequencing: Plan 31 executes on UI code; Plan 29 executes on security telemetry. They can run in parallel; no shared files.
