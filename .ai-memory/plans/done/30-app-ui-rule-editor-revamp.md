...
- Command: `.ai-memory/02-spec/commands/01-fonts-ubuntu-poppins.md`
- Issue: `.ai-memory/issues/09-setup-ui-not-modern.md`
- Reference image: https://io.eklas.dev/media/18706c4d1a9c/2026/07/14/1784056065593_kak2n7wvouyp_image.png
- Existing UI DS spec: `02-spec/24-app-ui-design-system/00-overview.md`
- Core DS: `02-spec/07-design-system/`

## Steps

### Spec — one file per concern (1–20)

1. Audit `02-spec/07-design-system/` and list every semantic token the editor will reuse; write findings to `02-spec/24-app-ui-design-system/_notes/token-inventory.md`.
2. Audit current `src/routes/setup*.tsx` + `src/components/hmi/**`; list what gets deleted vs kept in `_notes/impl-inventory.md`.
3. Write `02-spec/24-app-ui-design-system/01-foundations.md` covering typography (Ubuntu display, Poppins body, weights, sizes), color tokens (canvas surface, overlay line, rule idle/hover/selected/error), spacing scale (4/8/12/16/24/32), elevation tiers, motion (200 ms fade+scale, reduced-motion fallback), and a11y baseline (WCAG 2.1 AA, focus ring token).
4. Add the icon table (rule-kind → lucide icon) as a short section in `01-foundations.md`; do not create a separate file.
5. Write `02-spec/24-app-ui-design-system/02-layout.md` covering shell (top bar, tab strip, workspace, right rail), floating tool ribbon docked bottom-center, collapsible lighting drawer, status strip. Include one ASCII wireframe.
6. Write `02-spec/24-app-ui-design-system/03-canvas.md`: full-bleed image, zoom-to-cursor 0.25×–8×, Space+drag pan, `F` fit, `1` 100%, view state not persisted.
...
10. Write `02-spec/24-app-ui-design-system/04-rule-layers.md` covering the Rule List (right rail top): drag-to-reorder, eye, lock, inline rename, thumbnail, kind badge.
11. Write `02-spec/24-app-ui-design-system/05-rule-controller.md` — link SS-01 for the normative schema. See `./subtasks/30-app-ui-rule-editor-revamp/ss-01-rule-controller-schema.md`. Include one table listing all rule kinds with a one-line parameter summary each; do not create per-kind files.
12. Add "OCR / Text-match / Math expression" sample notation to `05-rule-controller.md` with 2–3 worked examples.
13. Write `02-spec/24-app-ui-design-system/06-state-persistence.md`: Zustand store shape, serialize to `programs/*.json` via existing SettingsStore, forward-only migration note, 50-step undo stack, coalesce drag ops.
14. Write `02-spec/24-app-ui-design-system/07-errors-logging.md`: register `E_UI_RULE_INVALID`, `E_UI_CANVAS_LOAD`, `E_UI_LIGHTING_APPLY`, `W_UI_RULE_UNSAVED`; every state transition emits one structured log with `correlation_id`; every route has `errorComponent` + `notFoundComponent`.
    14a — reserved (kept out to keep numbering).
15. Cross-link the new error codes from `02-spec/03-error-manage/` code registry (append, don't fork).
16. Write `02-spec/24-app-ui-design-system/08-testing.md`: unit list (coords, hit-test, undo, math evaluator), Playwright flows, Axe gate (zero color-contrast fails on `/setup*`), perf budget (16 ms frame with 200 rules, 200 ms panel open), visual snapshots at 1440×900 and 1024×768.
17. Update `02-spec/24-app-ui-design-system/00-overview.md` Document Inventory to list the 8 new files.
18. Update `02-spec/spec-index.md` with the new tree.
19. Peer-review pass: walk every new file, fix broken cross-links; freeze v1.
20. Delete `_notes/` scratch files or move them under `02-spec/24-app-ui-design-system/_archive/` so the shipping spec is clean.
...
26. Define the "spec is done" checklist inside `02-spec/24-app-ui-design-system/00-overview.md` (Impl blocked until every checkbox is ticked).
...
35. Tag v1.0 of the UI DS in `02-spec/24-app-ui-design-system/00-overview.md` frontmatter.
...
99. Update `.ai-memory/memory/04-design-system.md` and `.ai-memory/memory/index.md` with landing evidence + screenshots.
100.  `mv .ai-memory/plans/pending/30-app-ui-rule-editor-revamp.md .ai-memory/plans/done/30-app-ui-rule-editor-revamp.md` and flip `Status:` to `completed`.

## Verification

- Spec: exactly 8 files under `02-spec/24-app-ui-design-system/` (01–08) plus `00-overview.md`. Any additional file is a review failure.
