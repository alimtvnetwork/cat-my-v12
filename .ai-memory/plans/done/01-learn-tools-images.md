...
No prior pending plans, commands, or issues found under `.ai-memory/` (only `project.json` existed).

## Steps

1. Enumerate every file in `assets/tools-images/`, separating real JPGs from `.asset.json` pointers, and write the inventory to `.ai-memory/plans/subtasks/01-learn-tools-images/ss-01-inventory.md`.
...
13. Write the consolidated brief to `.ai-memory/plans/subtasks/01-learn-tools-images/SS-02-design-brief.md` (screens, components, tokens, flows, open questions).
...
15. Move this plan file from `pending/` to `.ai-memory/plans/done/01-learn-tools-images.md` and flip `Status:` to `completed` once steps 1–14 have artifacts on disk.
