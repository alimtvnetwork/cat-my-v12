...
- `02-spec/21-app/40-error-manage.md` (W_SEC_DENIAL_BURST row)

No new user commands or issues were emitted in this planning turn (template-only "20 steps" instruction).

## Steps

1. Read `.ai-memory/plans/pending/29-denial-burst-threshold-tuning.md` in full; enumerate the 50-step surface and mark which of steps 1-15 are already partially landed under `subtasks/29-denial-burst-threshold-tuning/`.
2. Read every file under `.ai-memory/plans/subtasks/29-denial-burst-threshold-tuning/` and produce a short landed-vs-open matrix at `.ai-memory/memory/v2/plan29/00-baseline-gap.md`.
3. Read `02-spec/21-app/40-error-manage.md` A.1 Security table and pin the current `W_SEC_DENIAL_BURST` contract (emitter, subject, detail schema) into the baseline-gap memo.
...
7. Write `.ai-memory/memory/v2/plan29/10-telemetry-inventory.md` listing every burst-detection input (rate-limit denials, role denials, retention failures) with path:line refs.
...
13. Add `scripts/security/plan29_windows.py` that reads exporter JSONL and writes `.ai-memory/memory/v2/plan29/20-windows.json` (per-window percentiles + row counts + first/last ts).
...
15. Write `.ai-memory/memory/v2/plan29/30-derivation-inputs.md`: table of candidate thresholds (current vs p95 vs p99) with the arithmetic shown, no decision yet.
16. Update `.ai-memory/plans/pending/29-denial-burst-threshold-tuning.md` step 1-15 checklist: flip each landed step to `[x]` with a citation to this plan's file/line.
17. Update `.ai-memory/memory/index.md`: add a `Plan 29 read+data phase (v3.93.0)` block linking the three memo files and `20-windows.json`.
...
20. Move this file to `.ai-memory/plans/done/33-plan-29-denial-burst-tuning-read-phase.md` and flip `Status: pending` -> `Status: completed`; leave parent Plan 29 in `pending/` (derivation phase still open).

## Verification

- Steps 1-7: `ls .ai-memory/memory/v2/plan29/` shows `00-baseline-gap.md`, `10-telemetry-inventory.md`.
...
- Steps 16-17: `rg "Plan 29 read\+data" .ai-memory/memory/index.md` returns the block; parent plan checklist reflects landed steps.
- Steps 18-19: `grep 3.93.0 package.json readme.md changelog.md release_notes.md` returns 4 matches.
- Step 20: `ls .ai-memory/plans/pending/33-*.md` empty; `ls .ai-memory/plans/done/33-*.md` present; parent Plan 29 still in `pending/`.

## Appended from prior pending tasks

- Plan 29 (rank 4) full surface: derivation (steps 16-25), migration/code (26-35), tests (36-42), spec+observability (43-48), close-out (49-50) all remain in `.ai-memory/plans/pending/29-denial-burst-threshold-tuning.md`. This plan closes only steps 1-15.
- Plan 32 (SG-31-01 PatternEdge, 10 steps) remains in `.ai-memory/plans/pending/32-sg-31-01-pattern-edge.md`; no dependency on this plan.
- Housekeeping: remove residual "pre-93 panel gaps" line from `.ai-memory/memory/index.md` once Plan 32 lands (out of scope here).

## Closure (v3.236.0)

All steps landed as of v3.235.0 (see `.ai-memory/memory/v2/plan33/00-read-phase-status.md`). Moved to `done/` in this slice. Threshold-change follow-up (plans 29, 49) remains parked pending real 90-day telemetry (>= 200 events); see `.ai-memory/memory/v2/plan29/40-read-phase-status.md`.
