...
Second executable slice of `.ai-memory/plans/pending/43-coding-quality-error-dialog-and-mode-flag.md`, running after slice 1 (`44-plan43-execution-slice-1.md`) lands the `AppMode` flag and `src/lib/constants/**` scaffolding. This slice migrates existing call sites to the shared constants, applies the boolean/control-flow readability rules, and syncs the affected spec docs. No feature or logic change. Related commands: `.ai-memory/02-spec/commands/20-pascalcase-no-magic-strings.md`, `.ai-memory/02-spec/commands/21-code-quality-boolean-and-flow.md`.
...
4. Sync spec files touched by the renames: `02-spec/02-coding-guidelines/02-typescript/**`, `02-spec/02-coding-guidelines/08-file-folder-naming/**`, `02-spec/03-error-manage/02-error-architecture/04-error-modal/*`, `02-spec/24-app-ui-design-system/03-canvas.md`, and `02-spec/21-app/61-sample-images-and-focus.md`; cross-link every renamed identifier to its constants module.
