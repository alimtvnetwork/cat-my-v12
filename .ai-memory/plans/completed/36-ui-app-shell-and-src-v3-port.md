...
created: 2026-07-16
revised: 2026-07-16
subtasks_dir: .ai-memory/plans/subtasks/36-ui-app-shell-and-src-v3-port/
---

# Plan 36 - UI App-Shell Overhaul + src_v3 Port

Goal: turn the current "website" home into a Windows-style desktop HMI shell (titlebar row + menubar with 8 dropdown groups + status bar + `Cmd/Ctrl+K` command palette), replace the marketing tile grid with the real Jobs + Tasks operator surface, and port every reusable surface from `src_v3/` file by file. Exactly 50 atomic steps. Do not execute this plan the same turn it is written (per `.ai-memory/02-spec/commands/01-plan-50-workflow.md`).
...
3. **Register plan in memory index.** Why: `.ai-memory/memory/index.md` is the always-read entry point; without a row here, the next agent will miss that plan 36 is in flight and may collide. Append one row pointing at this file. Verify: row present, no other rows mutated.
...
50. **Archive.** Why: plan-workflow spec says archived plans move to `done/` with `Status: completed`. Move this file to `.ai-memory/plans/done/36-ui-app-shell-and-src-v3-port.md`, flip `status:` frontmatter, append a closure row to `.ai-memory/memory/index.md`. Verify: `pending/` no longer contains plan 36; memory index closure row present.
