...
Based on a codebase-wide audit against `.ai-memory/coding-guidelines.md`, there are widespread violations that must be fixed via the looping agent. Attempting to fix them all in one go is unfeasible. This plan chunks the remediation into smaller slices so the agent can loop through them.
