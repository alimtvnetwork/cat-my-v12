...
User asked for a per-image explainer MD written for an AI reader, describing every portion of each of the 50 screenshots in `assets/tools-images/` (KEYENCE-style vision inspection HMI): layout, colors, text, controls, purpose, expected user actions, and what each button does. Output lives in the spec UI folder at `02-spec/24-app-ui-design-system/tools-images/` (folder 24 = app UI design system). Command captured at `.ai-memory/02-spec/commands/15-tools-images-explainers.md`. No new issues.
...
1. Document `01-hmi-main-run-screen-measurement-list.jpg` → `02-spec/24-app-ui-design-system/tools-images/01-hmi-main-run-screen-measurement-list.md`. See `./subtasks/40-tools-images-spec-docs/00-md-template.md`.
...
50. Document `50-utility-menu-batch-test-monitor-settings.jpg` → `50-utility-menu-batch-test-monitor-settings.md`, then write `02-spec/24-app-ui-design-system/tools-images/INDEX.md` listing all 50 entries with one-line summaries and linking the shared template.

## Verification

- All 50 target MD files exist under `02-spec/24-app-ui-design-system/tools-images/` and each is at least ~120 lines covering the 10 required sections from the template.
...
- `02-02-spec/99-consistency-report.md` gets a short entry noting the new sub-folder (added in the same execution turn as step 50).

## Appended from prior pending tasks

None pulled in — existing pending plans (29, 32, 33, 35–39) are independent tracks and remain in `.ai-memory/plans/pending/`.
