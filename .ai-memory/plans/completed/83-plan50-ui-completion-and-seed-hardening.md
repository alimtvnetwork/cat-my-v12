...
Applies coding-guidelines and error-manage rules from `02-spec/` and
...
3. Reconcile open issues `.ai-memory/issues/28`..`34` against grep evidence: mark each as fixed/partial/still-open with a one-line justification.
...
6. Cross-link `docs/plan-83/pending-audit.md` from `02-spec/21-app/53-ui-improvements-v4.md` under a "Plan 83 audit" heading.
7. Update `.ai-memory/memory/index.md` with a one-line pointer to Command 36 (top-tier UI craft baseline) so future sessions apply the rule without re-reading this plan.
...
48. Update `02-spec/21-app/53-ui-improvements-v4.md` "Status" line to note Plan 83 completion and the closed-out items from Plans 79/80/81/82.
49. Move each of Plans 79, 80, 81, 82 from `.ai-memory/plans/pending/` to `.ai-memory/plans/completed/` using `mv`, flipping their `Status:` frontmatter to `completed` if and only if the audit in step 4 shows every remaining item was absorbed into Plan 83 and shipped. Any plan with still-open unabsorbed work stays in `pending/` with a note appended.
50. Bump minor version, update `CHANGELOG.md` + `RELEASE_NOTES.md`, pin the final version in `README.md`, and `mv` this plan file to `.ai-memory/plans/completed/83-plan50-ui-completion-and-seed-hardening.md` with `Status: completed`.
...
- [~] 1-8: audit + docs. Audit outputs live at `.ai-memory/plans/subtasks/84-next-20-onboarding-and-pending-drive/SS-01…SS-07.md` and `.ai-memory/plans/subtasks/83-plan50-ui-completion-and-seed-hardening/SS-01-pending-audit-report.md` rather than `docs/plan-83/`. Cross-link into `02-spec/21-app/53-ui-improvements-v4.md` (step 6) and memory-index update (step 7) NOT done. Version pin bump (step 8) done incrementally each turn.
