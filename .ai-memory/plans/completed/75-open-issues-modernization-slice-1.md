...
Splits `.ai-memory/plans/pending/74-open-issues-modernization-sweep.md` (30 coarse steps) into a tighter 20-step plan focused on the six deferred open issues from Plan 73 closeout (`.ai-memory/memory/v2/plan73/44-issue-sweep.md`, `.ai-memory/memory/v2/plan73/90-closeout.md`):
...
Plan 74 is superseded on completion of step 20. Guideline sources to re-digest: `.ai-memory/coding-guidelines.md`, `02-spec/03-error-manage/**`, `.ai-memory/memory/03-error-manage.md`, `.ai-memory/memory/04-design-system.md` (Plan 73 closeout tokens: `--ca-on-primary`, tinted-primary pill pattern).

No new commands or issues emitted this turn (template-only header, scope answered via clarifying question).

## Steps

1. Digest guidelines + design tokens into `.ai-memory/memory/v2/plan75/00-guideline-digest.md` (coding-guidelines, error-manage, Plan 73 tokens).
2. Read issues 09, 11, 12, 13, 14, 15 end-to-end; write `.ai-memory/memory/v2/plan75/01-issue-map.md` with symptom, repro, suspect files, severity, and per-issue acceptance criteria.
3. Capture baseline screenshots (1280x1800, `comfortable` + `compact` header density) for `/`, `/setup`, `/setup/roi`, `/setup/reference`, `/setup/rules` under `/tmp/browser/plan75/baseline/`; commit inventory to `.ai-memory/memory/v2/plan75/02-baselines.md`.
4. Issue 13 fix: restore home launcher on `/` (Projects, Setup, Trial run, AI testing entries) per `.ai-memory/02-spec/commands/09-home-hub-top-nav.md`; keep frontend-only.
...
7. Verify 13/15/14 with Playwright: nav from `/setup`, `/projects`, `/setup/rules` back to `/` via Home affordance; screenshot each; write `.ai-memory/memory/v2/plan75/07-nav-verify.md`.
...
15. Header-density verification: toggle `comfortable` / `compact` on all touched routes; screenshot; ensure no overlap or clipping. Write `.ai-memory/memory/v2/plan75/15-density-check.md`.
16. Run combined gate: `bunx tsgo --noEmit`, `bunx vitest run`, `python3 tests/e2e/axe_a11y.py`, `bun run visual:test`. Record output in `.ai-memory/memory/v2/plan75/16-gate.md`.
17. Regenerate Plan 69 visual baselines only if intentional diffs land in steps 4-14; document deltas in `.ai-memory/memory/v2/plan75/17-visual-refresh.md`.
18. Flip `Status: open` -> `Status: closed` in `.ai-memory/issues/{09,11,12,13,14,15}-*.md`; add a Closed-by frontmatter line pointing at Plan 75.
19. Supersede Plan 74: `mv .ai-memory/plans/pending/74-open-issues-modernization-sweep.md .ai-memory/plans/completed/` with `Status: superseded-by-plan-75`; move Plan 75 to `completed/` and flip its status; write `.ai-memory/memory/v2/plan75/90-closeout.md`.
20. Version bump + README pin (v3.511.0), append CHANGELOG + RELEASE_NOTES entries citing the six closed issues.

## Verification

- Step 1-3: memory files present under `.ai-memory/memory/v2/plan75/`.
...
- Step 18: `rg "Status: open" .ai-memory/issues/{09,11,12,13,14,15}-*.md` returns empty.
...
- `.ai-memory/plans/pending/74-open-issues-modernization-sweep.md` (superseded by this plan at step 19).
