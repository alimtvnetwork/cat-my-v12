...
- Issue: `.ai-memory/issues/11-layers-mixed-with-detector-controls.md`
- Issue: `.ai-memory/issues/12-ui-overlap-and-density.md`
- Command: `.ai-memory/02-spec/commands/10-photoshop-layers-and-drag-drop.md`
- Command: `.ai-memory/02-spec/commands/11-ui-density-guardrails.md`

Guideline sources honored:

- `02-spec/coding-guidelines/{typescript,python,sql}.md`
- `coding-guidelines/00-overview.md` + `02-coding-guidelines/`, `07-design-system/`
- `02-spec/03-error-manage/` (error architecture + registry)
- `02-spec/21-app/40-error-manage.md` Appendix A (wire codes)

## Steps

1. Read `02-spec/03-error-manage/{01-error-resolution,02-error-architecture,03-error-code-registry}.md` and `02-spec/21-app/40-error-manage.md` A.1; pin any new wire codes needed for layers/properties (`E_LAYER_REORDER_FAILED`, `E_LAYER_MERGE_INCOMPATIBLE`, `W_LAYER_GROUP_EMPTY`) into `.ai-memory/memory/v2/plan35/00-error-contract.md`.
2. Read `coding-guidelines/07-design-system/` and pin the density tokens (`--spacing-hmi-*`, row height mins, border rules) actually used today into `.ai-memory/memory/v2/plan35/01-design-tokens.md`.
3. Read `src/lib/rules-slice.ts` + `src/lib/editor-slice.ts` + tests under `src/lib/__tests__/` (if any) and write `.ai-memory/memory/v2/plan35/02-store-shape.md` listing every rule/selection action already in the store.
4. Read `src/components/editor/rail/{RightRail,RuleList,RuleRow,CircleRuleEditor,RectRuleEditor,OcrRuleEditor,TextRuleEditor,MathRuleEditor}.tsx` and `src/components/editor/panels/*` and write `.ai-memory/memory/v2/plan35/03-current-rail.md`: what renders where today.
5. Density audit: run Playwright captures for the 19 screens listed in SS-04 at 1280x800 and 1920x1080 to `/tmp/browser/plan35/*.png`; write `.ai-memory/memory/v2/plan35/04-density-audit.md` (see ./subtasks/35-ui-ux-photoshop-layers-overhaul/SS-04-density-audit.md).
...
24. Update `02-spec/21-app/40-error-manage.md` A.1 to register the three new wire codes from step 1 with subject/detail schema; append changelog to `02-spec/03-error-manage/98-changelog.md`.
25. Update `02-spec/21-app/**` rule-editor spec (or nearest active file) with a "Layers vs Properties" section and drag/drop/group/merge contract; reference Command 10.
26. Update `.ai-memory/memory/index.md` with a "Plan 35 UI overhaul (v3.138.0)" block linking the memos + `20-audit.md` screenshots.
...
30. Move this file to `.ai-memory/plans/done/35-ui-ux-photoshop-layers-overhaul.md` and flip `status: completed` -> `Status: completed`; leave Plan 29/32/33 in `pending/` (unaffected).

## Verification

- Steps 1-4: `ls .ai-memory/memory/v2/plan35/` shows 4 memo files.
...
- Step 30: `ls .ai-memory/plans/pending/35-*` empty; `ls .ai-memory/plans/done/35-*` present.

## Appended from prior pending tasks

- Plan 29 (`.ai-memory/plans/pending/29-denial-burst-threshold-tuning.md`): derivation 16-25, migration/code 26-35, tests 36-42, spec+observability 43-48, close-out 49-50. Not touched by Plan 35.
- Plan 32 (`.ai-memory/plans/pending/32-sg-31-01-pattern-edge.md`): SG-31-01 PatternEdge, 10 steps. Not touched by Plan 35.
- Plan 33 (`.ai-memory/plans/pending/33-plan-29-denial-burst-tuning-read-phase.md`): steps 15-20 remaining (derivation-inputs memo, parent checklist flip, memory index, docs, move to done). Not touched by Plan 35.
