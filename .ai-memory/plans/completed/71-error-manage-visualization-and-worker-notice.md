...
Worker-offline floating card is visibly clipped and overall error UI does not follow `02-spec/03-error-manage/`. This plan aligns notifications, error modal, error history, and error-code registry with the spec, and enforces "hide when clipped" for floating notices.

Linked:

- Issue: `.ai-memory/issues/25-worker-notice-cut-and-poor-error-visualization.md`
- Command: `.ai-memory/02-spec/commands/25-hide-clipped-floating-notices.md`
- Spec root: `02-spec/03-error-manage/`

## Steps

1. Read every file under `02-spec/03-error-manage/01-error-resolution/`, `02-error-architecture/`, `03-error-code-registry/` and produce a short compliance-gap note at `./subtasks/71-error-manage-visualization-and-worker-notice/SS-01-spec-gap-audit.md`.
...
5. Restyle the worker notice using notification tokens from `02-spec/03-error-manage/02-error-architecture/03-notification-colors.md` (`--toast-error-*`) instead of ad-hoc classes.
...
11. Integrate error-code registry: import `02-spec/03-error-manage/03-error-code-registry/error-codes-master.json` into `src/lib/errors/registry.ts` with typed lookup.
...
19. Update `02-spec/03-error-manage/97-acceptance-criteria.md` and `99-consistency-report.md` to reflect the shipped implementation.
20. Run `scripts/ci-v3.sh` green, bump version, then `mv` this plan file into `.ai-memory/plans/completed/71-...md` and flip `Status: completed`.
...
None (unrelated pending plans 29/35/36/41/43/50 remain in `.ai-memory/plans/pending/`).
