...
2. Spec first, then plan. Order is fixed:
   ...
   a. Write the spec task file(s) at the project's declared spec path, or `.ai-memory/02-spec/tasks/XX-<slug>.md` if none is declared. Each spec file states intent, scope, inputs, acceptance criteria, affected files, and links to captured commands / issues / resolved ambiguities / attachments.

   b. Write the plan at `.ai-memory/plans/pending/XX-<slug>.md`. Every step references the spec task file it implements.
...
- New plan: write to `.ai-memory/plans/pending/XX-<slug>.md` with `Status: pending`. Update `.ai-memory/plans/index.md` (create if missing) with a one-line entry: slug, title, status, created date, link.

- Done: `mv` to `.ai-memory/plans/completed/XX-<slug>.md`, flip `Status: completed` in the same move, update `plans/index.md`. Never copy. Never duplicate.
...
- File: `.ai-memory/plans/subtasks/XX-<slug>/SS-<subslug>.md` with `Parent: XX-<slug>` in frontmatter.
...
| Command, new convention, "always do X", new CLI | `.ai-memory/02-spec/commands/XX-<slug>.md` |

| Bug, regression, broken behavior | `.ai-memory/issues/XX-<slug>.md` |

| CI/CD-specific failure | `.ai-memory/cicd-issues/XX-<slug>.md` |

| Institutional knowledge (pattern, convention, decision) | `.ai-memory/memory/` + update `memory/index.md` |
...
1. Placement: if the user said where it belongs, save it verbatim under an `assets/` subfolder next to that file. Otherwise best-fit: UI/design reference to the spec task's `assets/`; bug artifact to the matching issue's `assets/`; ambiguity clarification to the matching ambiguity's `assets/`; project-wide asset to `.ai-memory/assets/<slug>/` and note in `memory/index.md`. When in doubt, current task's spec `assets/`.
...
Read every location that exists; skip silently when missing. On conflict, prefer numeric `spec/NN-…/` folders over generic `.ai-memory/*.md` and call the conflict out in Context.

Coding tasks (Go, Python, PHP, TS, any backend):

- `.ai-memory/coding-guidelines.md`

- `02-spec/02-coding-guidelines/` or `02-spec/coding-guidelines/`

- `coding-guidelines/` at repo root

- Error-management (mandatory for coding tasks): `02-spec/03-error-manage/`, `02-spec/XX-error-manage/`, `coding-guidelines/XX-error-manage/`
...
- [ ] Scanned `.ai-memory/` recursively; read `plans/index.md`, every `pending/` file, `memory/index.md` and referenced files, every open ambiguity, relevant `spec/NN-*/`, error-management specs for code tasks; skimmed `completed/`
...
- [ ] Plan saved to `.ai-memory/plans/pending/XX-<slug>.md`; Context links every spec / command / issue / resolved ambiguity / attachment
...
- [ ] Subtask files under `.ai-memory/plans/subtasks/XX-<slug>/` where depth was needed

- [ ] `.ai-memory/plans/index.md` updated (created if missing)
