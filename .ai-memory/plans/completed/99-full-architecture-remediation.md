...
Spec task: `.ai-memory/02-spec/tasks/99-full-architecture-remediation.md`
Subtasks:

- Phase A: `.ai-memory/plans/subtasks/99-full-architecture-remediation/SS-01-backend-split-brain.md`
- Phase B: `.ai-memory/plans/subtasks/99-full-architecture-remediation/SS-02-route-directory-migration.md`
- Phase C: `.ai-memory/plans/subtasks/99-full-architecture-remediation/SS-03-store-localization.md`
- Phase D: `.ai-memory/plans/subtasks/99-full-architecture-remediation/SS-04-backend-bootstrap-docs.md`

Coding guidelines: `.ai-memory/coding-guidelines.md` (15-line function cap, 100-line .tsx cap,
no magic strings, enum-only bounded sets, positive boolean names, no nested if).
Strictly avoid: `.ai-memory/strictly-avoid.md` (no string unions, Enum must have Type suffix,
positive booleans, no TS2322/TS2339 left unaddressed).
Memory: `.ai-memory/memory/13-avoid-blind-mass-refactors.md` (NO untargeted AST mass-replace).
Prior plan: `.ai-memory/plans/completed/98-architecture-consolidation-improvements.md`
...
37. Update `.ai-memory/plans/subtasks/99-full-architecture-remediation/SS-01-backend-split-brain.md`: flip `Status: completed` to `Status: completed`. Spec: plan lifecycle rule.
...
117.  Update `.ai-memory/plans/subtasks/99-full-architecture-remediation/SS-02-route-directory-migration.md`: flip `Status: completed` to `Status: completed`. Spec: plan lifecycle.
...
150. Update `.ai-memory/plans/subtasks/99-full-architecture-remediation/SS-03-store-localization.md`: flip `Status: completed` to `Status: completed`. Spec: plan lifecycle.
...
161. Run `grep -rn "BE/routes" .ai-memory/ --include="*.md"`. Record every hit. Spec: same.
...
164. Read `.ai-memory/memory/index.md` lines 1-123 for any `BE/routes/` references. Correct them. Spec: `SS-04 §SS-04-07`.

165. Read `.ai-memory/overview.md` for any `BE/routes/` references in architecture diagrams or text. Correct them. Spec: `SS-04 §SS-04-08`.
...
170. Update `.ai-memory/plans/subtasks/99-full-architecture-remediation/SS-04-backend-bootstrap-docs.md`: flip `Status: completed` to `Status: completed`. Spec: plan lifecycle.

### PHASE E — SPEC ALIGNMENT & CROSS-CUTTING CLEANUP

171. Read `02-spec/21-app/backend-implementation-request-v1.md` (referenced in `BE/main.py` docstring). Confirm it reflects the current router structure (no `BE/routes/` references). If stale, update it. Spec: `99-full-architecture-remediation.md §Acceptance Criteria 7`.

172. Read `.ai-memory/plans/architecture-and-code-observations.md`. Confirm it is marked as superseded by Plan 99 or still accurate. Append a note at the bottom: `## Plan 99 — Remediation complete (2026-08-17): split-brain resolved, routes migrated, stores localized, Makefile added.` Spec: same.

173. Update `.ai-memory/plans/index.md`: add the Plan 99 entry to the Pending section: `- 99 - full-architecture-remediation - pending - see pending/99-full-architecture-remediation.md`. Spec: plan lifecycle rule.

174. Read `.ai-memory/memory/30-architecture-observations.md` (if it exists). If it has stale notes about `BE/routes/` or flat route files, update them. Spec: same.
...
179. Read `.ai-memory/coding-guidelines.md` §12 (asset sequencing rule). Confirm no assets were created by this plan that violate the `assets/NN-folder/NN-file.ext` naming rule. Spec: `coding-guidelines.md §12`.
...
187. Check that no file created or modified by this plan violates the 300-line file cap (`.ai-memory/coding-guidelines.md §6`). Run `Get-ChildItem src/contexts, BE/routes, BE/models -Filter "*.py","*.ts","*.tsx" | ForEach-Object { $c = (Get-Content $_.FullName).Count; if ($c -gt 300) { Write-Host "$($_.Name): $c lines" } }`. Output must be empty. Spec: same.
...
189. Check `.ai-memory/strictly-avoid.md`. Confirm no violations: no string union types in new files, no Enum without `Type` suffix in new files, no implicit truthiness checks in new files. Spec: `.ai-memory/strictly-avoid.md`.
...
193. Update `.ai-memory/suggestions.md`: move `S-98` row from "Active Suggestions" to "Implemented Suggestions" (it was the architecture consolidation suggestion that this plan completes). Spec: plan lifecycle.

194. Update `.ai-memory/memory/index.md`: add a new entry row for Plan 99 completion: `| [plans/completed/99-full-architecture-remediation.md] | Plan 99 remediation: BE split-brain fixed, routes migrated to dirs, stores localized, Makefile added | Before any BE or routing work |`. Spec: memory maintenance.

195. Update `.ai-memory/memory/06-spec-map.md` if it references `BE/routes/` or old route structure. Spec: same.
...
197. Move this plan file from `.ai-memory/plans/pending/99-full-architecture-remediation.md` to `.ai-memory/plans/completed/99-full-architecture-remediation.md`. Flip `Status: completed` to `Status: completed`. Never copy — move only. Spec: plan lifecycle rule.

198. Update `.ai-memory/plans/index.md`: move the Plan 99 entry from Pending to Completed section: `- 99 - full-architecture-remediation - completed - see completed/99-full-architecture-remediation.md`. Spec: same.
