...
Implement the "UI Improvements V4" pass captured in `02-spec/21-app/53-ui-improvements-v4.md` and `.ai-memory/02-spec/commands/28-ui-improvements-v4.md`. Covers the Photoshop-style Tools + Properties + Layers palettes, rotatable selection overlays with legible badges, unified Rule/Category editor with `appliesBefore` chain, Project CRUD with rule-chain expansion, CameraSetting binding, MicSettings stub, and mandatory seed + facade wiring. Reference images live at `02-spec/21-app/instruction-images-v4/01..05.png`. Related issue: `.ai-memory/issues/27-properties-panel-and-badges-crappy.md`.
...
- Reference images render inside `02-spec/21-app/53-ui-improvements-v4.md` via relative links.
...
- Every new facade has a matching TODO under `.ai-memory/pending-facades/`.

## Appended from prior pending tasks

Scan of `.ai-memory/plans/pending/` on 2026-07-18 shows these older pending plans; they are unrelated to V4 scope and remain in `pending/` for their own owners:
