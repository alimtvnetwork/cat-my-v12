...
User asked for a 20-step plan after reading the codebase, `02-spec/`, and
`.ai-memory/` memory to understand structure and outstanding work. Scan of
`.ai-memory/plans/pending/` shows 22 open plans; the highest-priority open
...
- `.ai-memory/02-spec/commands/07-plan-from-backlog-top.md`
- `.ai-memory/02-spec/commands/08-plan-lifecycle-done-folder.md`
- `.ai-memory/02-spec/commands/14-plan-30-read-spec-code-memory.md`
- `.ai-memory/02-spec/commands/36-top-tier-ui-craft-baseline.md`

Applies coding-guidelines + error-manage rules from `02-spec/` and
`.ai-memory/coding-guidelines.md`. Every user-visible failure MUST route
through `showToastError` + `useErrorStore.captureException`.

## Steps

1. Read `mem/index.md`, `.ai-memory/memory/index.md`, and every file under `.ai-memory/memory/` (core, design, features, v2, session digests) to reload project context; note deltas since last onboarding in `.ai-memory/plans/subtasks/84-next-20-onboarding-and-pending-drive/SS-01-memory-refresh.md`.
2. Walk `02-spec/` breadth-first (top-level folders only) and record one-line purpose per folder in the same subtask note; deep-dive `02-spec/02-coding-guidelines/`, `02-spec/03-error-manage/`, `02-spec/21-app/`.
...
4. Regenerate `.ai-memory/plans/AUDIT-2026-07-17-pending-inventory.md` in place with today's date, listing every file under `.ai-memory/plans/pending/` with status (blocked / ready / in-progress) and a one-line summary.
5. Cross-check `.ai-memory/issues/` (34 open) against pending plans; mark each issue with the plan that owns it or flag "orphan". Write to `./subtasks/84-next-20-onboarding-and-pending-drive/SS-02-issue-plan-map.md`.
6. Cross-check `.ai-memory/pending-facades/` (6 open) against pending plans and mark ownership the same way in the SS-02 map.
...
12. Execute Plan 83 step 9 (seed data landing on Home hub) using the swatches + project facades; verify against `.ai-memory/02-spec/commands/35-seed-fixtures-per-screen.md`.
...
15. Execute Plan 83 steps 15-17 (address-bar nav, tools-strip removal, duplicate breadcrumb fix) per `.ai-memory/02-spec/commands/32-address-bar-nav.md`; move `.ai-memory/issues/31-duplicate-breadcrumb.md` and `.ai-memory/issues/32-tools-strip-between-header-and-canvas.md` toward closure.
16. Execute Plan 83 steps 18-20 (padding/readability baseline pass, empty-state unification, saved-badge relative time) per `.ai-memory/02-spec/commands/31-padding-and-readability-baseline.md`.
...
19. Update `.ai-memory/plans/AUDIT-2026-07-17-pending-inventory.md` to reflect the new state; re-rank the top-of-queue for the next planning turn.
20. Move THIS plan file (`84-next-20-onboarding-and-pending-drive.md`) from `.ai-memory/plans/pending/` to `.ai-memory/plans/completed/` via `mv` and flip `Status: pending` to `Status: completed` in the frontmatter.
...
- Steps 8-11: Plan 83 subtasks folder contains fresh audit outputs; no source edits in these steps (git diff limited to `.ai-memory/`).
...
- Step 20: `.ai-memory/plans/completed/84-next-20-onboarding-and-pending-drive.md` exists with `Status: completed`, and no duplicate remains under `pending/`.
...
All 20 steps landed. Subtask notes: `.ai-memory/plans/subtasks/84-next-20-onboarding-and-pending-drive/SS-01..SS-19.md` plus this closeout.
...
Move: `mv .ai-memory/plans/pending/84-*.md .ai-memory/plans/completed/` performed at v3.787.0.
