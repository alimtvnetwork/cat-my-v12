...
3. Add a 24h shadow-compare script `scripts/security/plan29_shadow_compare.py` that reads the last 24h via the exporter and writes `.ai-memory/memory/v2/plan29/50-shadow-24h.json` comparing old-threshold vs new-threshold fire counts; commit a golden fixture run.
4. Update `02-spec/21-app/40-error-manage.md` A.1 Security row with the dashboard route link and the `W_SEC_DENIAL_BURST_ALERT` contract; append `.ai-memory/memory/v2/plan29/60-closeout.md` summarizing landed thresholds, shadow-compare deltas, and rollback steps.
5. Move `.ai-memory/plans/pending/29-denial-burst-threshold-tuning.md` (and 33, 47, 48, 49 once each verifies green) to `.ai-memory/plans/completed/` with `Status: completed`; verify `git diff --stat` shows only expected files and `tsgo --noEmit` + `vitest run` + the python test suite all exit 0.
...
- `.ai-memory/memory/v2/plan29/50-shadow-24h.json` exists with numeric deltas; `60-closeout.md` cites it.
