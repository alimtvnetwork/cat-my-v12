...
- 2026-08-13T00:00:00Z, Plan 98: Architecture & code observations + consolidation plan added under `.ai-memory/plans/architecture-and-code-observations.md` and `pending/98-architecture-consolidation-improvements.md`. Read before backend/runtime integration work.
...
1. `.ai-memory/what-to-read.md` (this file) - authoritative read-list pointer
2. [`.ai-memory/overview.md`](./overview.md) - **project overview**: architecture diagrams, repo map, what's good, recent work, gaps
3. [`.ai-memory/ai-improvement-guidelines.md`](./ai-improvement-guidelines.md) - **AI quality guide**: preserve/improve patterns, mandatory workflow, anti-patterns
4. `.ai-memory/memory/index.md` - master memory index; every memory file is listed here.
5. `.ai-memory/coding-guidelines.md` - coding rules and boundaries (hard caps, blind-follow)
6. `.ai-memory/plans/index.md` - active roadmap and completed items
   6b. `.ai-memory/plans/architecture-and-code-observations.md` - architecture review snapshot (Aug 2026); Plan 98 completed — see `completed/98-architecture-consolidation-improvements.md`
7. `.ai-memory/suggestions.md` and `.ai-memory/suggestions/index.md` - active and historical suggestions
8. `.ai-memory/strictly-avoid.md` - forbidden patterns
...
11. `.ai-memory/02-spec/commands/` - spec auditing and directives
12. `.ai-memory/ambiguous-questions/01-new-ambiguity/` and `02-ambiguity-resolved/`
13. `01-prompts/index.md` - prompt registry
14. `.ai-memory/memory/workflow/` - current workflow state
15. `02-spec/` (relevant folders including error management)
...
4. [Shell Implementation Status](../02-spec/21-app/shell/03-implementation-status.md)
...
├── .ai-memory/                     AI control tree (memory, plans, prompts, spec commands)
...
│   ├── 02-spec/commands/            Reusable spec/audit command prompts
...
| Before writing code                 | `.ai-memory/overview.md` (§ relevant), `.ai-memory/ai-improvement-guidelines.md`, `.ai-memory/memory/01-code-red.md`, `.ai-memory/coding-guidelines.md`, `02-spec/02-coding-guidelines/` |
| Architecture / backend integration  | `.ai-memory/overview.md` §2–4, `docs/architecture/runtime-map.md`, section 1b below                                                                                             |
| Before adding a feature             | `.ai-memory/memory/index.md`, latest `.ai-memory/plans/pending/*.md`, relevant `02-spec/21-app/*.md`, `02-spec/22-app-issues/`                                                          |
| Before adding a unit test           | `pytest.ini`, mirror layout under `tests/unit/`, follow `02-spec/04-database-conventions/04-testing-strategy.md` when DB-touching                                                |
| Before adding an integration test   | `tests/integration/` or `tests/e2e/` conventions; `tests/e2e/playwright_smoke.py` reference                                                                                   |
| Before writing a spec               | `02-spec/01-spec-authoring-guide/`, `spec/_template.md`, `.ai-memory/memory/06-spec-map.md`                                                                                        |
| DB schema / migration               | `02-spec/04-database-conventions/`, `02-spec/23-app-db/`, `app/core/io/migrations/`, GRANT + RLS rules in agent directives                                                          |
| UI change                           | `.ai-memory/memory/04-design-system.md`, `src/styles.css`, `02-spec/24-app-ui-design-system/`, `02-spec/07-design-system/`                                                            |
| Routing / server fn                 | `src/router.tsx`, `src/routes/readme.md`, TanStack Start rules in agent directives                                                                                            |
| Error handling                      | `.ai-memory/memory/03-error-manage.md`, `02-spec/03-error-manage/`, `app/core/errors/`                                                                                             |
| Naming (files, booleans, enums, DB) | `.ai-memory/memory/02-naming.md`, `.ai-memory/memory/09-enums-and-results-shape.md`                                                                                               |
| Vendor capture SDK work             | `.ai-memory/memory/08-vision-inspection-app.md`, `app/capture/vendor_*.py`, `02-spec/21-app/52-sdk-facade-pattern.md`                                                              |
| CI / linter change                  | `.ai-memory/memory/05b-linters.md`, `.ai-memory/memory/05c-linter-scripts.md`, `linter-scripts/`                                                                                  |
| Write/end memory                    | `01-prompts/` (write-memory prompt); write to files under `.ai-memory/`, never `mem://`                                                                                   |
...
- Never swallow errors. Follow `02-spec/03-error-manage/`.
- Never write memory to `mem://`. All persistent AI state lives under `.ai-memory/`.
...
- Every new file under `.ai-memory/memory/` MUST be added to `.ai-memory/memory/index.md` in the same operation.

## 5. Where to Write New Artifacts

| Artifact         | Path                                                      |
| ---------------- | --------------------------------------------------------- |
| New memory topic | `.ai-memory/memory/XX-topic.md` + index entry               |
| New plan         | `.ai-memory/plans/pending/XX-name.md`                       |
| Completed plan   | move to `.ai-memory/plans/done/`                            |
| Reusable prompt  | `01-prompts/XX-name.md` + `prompts/index.md`        |
...
- [`.ai-memory/overview.md`](./overview.md) - project overview with embedded Mermaid diagrams
- [`.ai-memory/ai-improvement-guidelines.md`](./ai-improvement-guidelines.md) - what to preserve, what to improve, AI workflow
- `.ai-memory/memory/07-lovable-folder-guide.md` - structural guide to `.ai-memory/`
- `.ai-memory/memory/06-spec-map.md` - spec folder map
...
1. Read `.ai-memory/memory/01-code-red.md` and `.ai-memory/coding-guidelines.md`.
2. Read the language-specific rules in `02-spec/02-coding-guidelines/` (TS, Python, SQL, etc.).
3. Read `02-spec/03-error-manage/` and `.ai-memory/memory/03-error-manage.md` before touching any error path.
4. Check `.ai-memory/plans/pending/` for an active plan on the area you are editing; if one exists, follow its slice.
...
2. Read `02-spec/04-database-conventions/04-testing-strategy.md` if the code touches the DB.
...
1. Read `.ai-memory/memory/index.md` and any plan under `.ai-memory/plans/pending/` that already covers the feature. If none exists, create `.ai-memory/plans/pending/XX-<name>.md`.
2. Cross-reference `02-spec/21-app/` (product spec) and `02-spec/22-app-issues/` (open gaps) for the feature area.
3. If the feature has UI, read `.ai-memory/memory/04-design-system.md`, `src/styles.css`, and `02-spec/24-app-ui-design-system/`.
4. If the feature has DB impact, read `02-spec/23-app-db/` and `02-spec/04-database-conventions/`, and add a migration under `app/core/io/migrations/`. Every new public table needs GRANT + RLS in the same migration.
5. Implement in small slices. Add unit tests in the same batch (see 7.2). Update the plan status.
6. When shipped, move the plan file from `.ai-memory/plans/pending/` to `.ai-memory/plans/done/`.

### 7.5 Write or edit a spec

1. Read `02-spec/01-spec-authoring-guide/` and `spec/_template.md`.
2. New spec files: `spec/<area>/XX-<slug>.md` with numeric prefix.
3. Register the file in `.ai-memory/memory/06-spec-map.md` in the same commit.
4. If the spec changes the roadmap, add or update a plan under `.ai-memory/plans/pending/`.

### 7.6 Persist memory (write-memory / end-memory)

1. Read `01-prompts/` for the write-memory prompt.
2. All persistent AI state goes under `.ai-memory/`. Never write to `mem://`.
3. New memory files: `.ai-memory/memory/XX-<topic>.md`; add the entry to `.ai-memory/memory/index.md` in the same operation.
4. Never delete history: mark done, move to `## Completed`, or move plans/issues to `done/` / `solved-issues/`.

### 7.7 Reusable prompt

1. New prompt: `01-prompts/XX-<name>.md`.
2. Register it in `01-prompts/index.md` with id, trigger phrases, and purpose.
3. `.ai-memory/prompt.md` stays a top-level pointer to `prompts/index.md`.

### 7.8 Fix a bug / track an issue

1. Open issues live under `.ai-memory/issues/XX-<slug>.md`; solved ones move to `.ai-memory/solved-issues/` with `## Solution`, `## Iteration Count`, `## Learning`, `## What NOT to Repeat`.
2. Recurring forbidden patterns go into `.ai-memory/strictly-avoid.md` pointing at the solved issue.
3. CI/CD issues: `.ai-memory/cicd-issues/XX-<slug>.md` with a one-line summary in `.ai-memory/cicd-index.md`.
