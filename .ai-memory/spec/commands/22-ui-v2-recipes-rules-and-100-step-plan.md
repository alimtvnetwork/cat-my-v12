...
Source: `02-spec/24-app-ui-design-system/09-UI-improvements-v2.md`
Captured: 2026-07-16
...
13. Database schema for rules, rule sets, categories, projects, camera settings, lighting settings, runs, and captures MUST be authored as Mermaid ER diagrams in `02-spec/23-app-db/` and rendered to PNG in the same folder. One mermaid per aggregate.
...
24. All ambiguity captured to `.ai-memory/ambiguity-questions/` as numbered files, one topic per file.
25. 100-step plan format is enforced per section "100 steps Plan, Maximal Enforcement" of the spec. One task = one file under `.ai-memory/plans/pending/`. Move to `.ai-memory/plans/done/` on completion, flip `Status:` frontmatter.
...
- All spec files in `02-spec/24-app-ui-design-system/` and `02-spec/23-app-db/`.
- All new SQLite migrations in `supabase/migrations` and any Cloud tables that back rules/projects.

## Related plans

- `.ai-memory/plans/pending/64-plan24-ui-v2-recipes-rules-and-desktop-overhaul.md`
