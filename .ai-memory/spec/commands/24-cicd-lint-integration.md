...
3. Any lint/format/typecheck script referenced in `.ai-memory/memory/05*` must be wired to the CI entrypoint. If a linter is added later, add it to the entrypoint the same commit.
4. The AI writes "CI: green (typecheck / lint / unit / e2e)" in the step's completion note. Missing evidence means the step reopens.
5. On a red pipeline the AI stops implementation, reports the failing stage verbatim, and either fixes the regression in the same turn or files an issue under `.ai-memory/issues/`.
