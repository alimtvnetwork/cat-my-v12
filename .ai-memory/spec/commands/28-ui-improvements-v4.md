...
1. Write V4 instructions into `02-spec/21-app/` as `53-ui-improvements-v4.md`. Store reference images under `02-spec/21-app/instruction-images-v4/` and reference them from the spec.
...
17. For every facade wired to a fake / IndexedDB implementation, log a pending TODO under `.ai-memory/pending-facades/` naming what needs to be swapped when the real SDK is provided.

## Plan-first rule

The next 50-step plan covering this command lives at `.ai-memory/plans/pending/79-ui-improvements-v4.md`. The first 10 steps document the design and image references so future turns can recall the intent.

## Related

- `.ai-memory/02-spec/commands/10-photoshop-layers-and-drag-drop.md`
- `.ai-memory/02-spec/commands/18-rule-condition-and-validation-order.md`
- `.ai-memory/02-spec/commands/22-ui-v2-recipes-rules-and-100-step-plan.md`
- `.ai-memory/02-spec/commands/23-photoshop-panels-window-menu.md`
- `.ai-memory/02-spec/commands/26-seed-via-facade-for-ui.md`
