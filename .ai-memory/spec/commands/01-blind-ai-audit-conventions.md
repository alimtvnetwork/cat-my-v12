...
Scope: All future audits of `02-spec/21-app/` and any new spec authoring.
When: From this command forward, until explicitly superseded.

Rules (verbatim from user, normalized):

1. The audit target audience is a "blind AI": a non-frontier model with only the `02-spec/` folder in context. No repo access, no chat history, no tribal knowledge.
...
7. Audit output layout in `02-spec/25-app-audit/`:
