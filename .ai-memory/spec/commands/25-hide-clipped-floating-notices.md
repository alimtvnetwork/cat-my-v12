...
If a floating notification's bounding box would extend beyond the viewport (horizontally or vertically) at the current layout, do NOT render it. Never show a half-clipped notice. Route the message through the Error Modal / toast stack per `02-spec/03-error-manage/` instead.
