...
Status: open
Filed: 2026-07-21
Source: `.ai-memory/plans/pending/90-worker-and-processing-cli.md` Step 1
Blocks: plan-90 Steps 8-70 (code should not encode a schema the spec contradicts)

## Context

Plan 90 Step 1 requires reading `02-spec/21-app/74-77` end-to-end and filing any spec inconsistencies before code lands. Doing the read surfaced six real contradictions between the four files. Each one would cause code + tests to diverge from a spec they claim to satisfy.

## Findings

### F1. JSONL field-name casing contradiction (74 vs 76)

- `02-spec/21-app/74-worker-cli.md:32` acceptance #3 lists fields as lowercase snake: `ts`, `level`, `event`, `code`, `msg`, `ctx`, `run_id`.
- `02-spec/21-app/76-cli-log-and-ipc.md:28-42` defines them PascalCase (`Ts`, `Level`, `Event`, `Code`, `Msg`, `Ctx`, `RunId`) and Rule 2 (`76:46`) says "Field names PascalCase (matches Universal Envelope convention)".
...
2. Close this issue by moving to `.ai-memory/issues/closed/` and appending a `## Resolution` block linking the patching commit.
3. No plan-90 step needs renumbering; findings only tighten the specs.

## Evidence

- `02-spec/21-app/74-worker-cli.md` (59 lines, read in full)
- `02-spec/21-app/75-processing-cli.md` (57 lines, read in full)
- `02-spec/21-app/76-cli-log-and-ipc.md` (152 lines, read in full)
- `02-spec/21-app/77-cli-powershell-and-release.md` (85 lines, read in full)
