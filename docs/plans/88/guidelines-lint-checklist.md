...
| 7   | Prompt aggregation for `01-prompts/` canonical mirrors          | Custom node script                      | `scripts/aggregate-prompts.mjs`                                   | `scripts/`                                                                               | manual + `ci.yml` check                                          |
...
| 12  | Universal Response Envelope + `AppError` shape (backend v1)           | tsgo (types) + unit tests               | `src/lib/facades/**` type guards                                  | `02-spec/21-app/backend-implementation-request-v1.md` (contract), tests under `tests/unit/` | `ci.yml`                                                         |
