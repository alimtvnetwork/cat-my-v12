...
The source module `02-spec/02-coding-guidelines/` contains **16 subfolders**. This consolidated reference covers every one of them. Use the section index below to jump directly to language- or topic-specific rules.
...
**Source:** `09-powershell-integration/` (cross-references to `02-spec/11-powershell-integration/` for the runtime)

The coding-side guidance is thin: PowerShell scripts that are **part of the build/CI pipeline** must follow `02-spec/11-powershell-integration/` (covered in `10-powershell-integration.md` consolidated file). For coding rules, only PascalCase Verb-Noun function naming and lowercase-kebab-case file naming apply.
...
Place research files here when the deliverable is a **decision that updates a coding rule** (e.g., "should we ban `interface{}` in Go?" → benchmark + memo). Otherwise, project-wide research goes to root `02-spec/10-research/` (covered in `12-root-research.md`).
...
These four subfolders mirror the root-level `02-spec/21-app/` … `02-spec/24-app-ui-design-system/` folders but contain **coding-specific** rules that only apply to the application layer (not the framework). They are intentionally minimal — most rules live in the cross-language section.

| Subfolder                  | What goes here                   | What does NOT                                                  |
| -------------------------- | -------------------------------- | -------------------------------------------------------------- |
| `21-app/`                  | App feature code style overrides | Feature requirements (those go in root `02-spec/21-app/`)         |
| `22-app-issues/`           | Bug-driven coding rule additions | Issue tickets (those go in root `02-spec/22-app-issues/`)         |
| `23-app-db/`               | App-specific schema patterns     | Generic DB rules (those go in `02-spec/04-database-conventions/`) |
| `24-app-ui-design-system/` | App component coding patterns    | Design tokens (those live in root `02-spec/07-design-system/`)    |
...
| 4   | `linter-scripts/check-spec-folder-refs.py`        | Python     | Verifies every `02-spec/NN-folder/` reference points to a real folder OR is allowlisted under `[external]` / `[doc-only]`                                    | `linter-scripts/spec-folder-refs.allowlist`                                                  | `0` pass · `1` stale reference                           | `python3 linter-scripts/check-spec-folder-refs.py`                           |
...
| 9   | `linter-scripts/generate-dashboard-data.cjs`      | Node       | Produces `02-spec/dashboard-data.json` consumed by the docs viewer                                                                                           | `version.json`, `spec/**/*.md`                                                               | `0` pass · non-zero on IO error                          | `node linter-scripts/generate-dashboard-data.cjs`                            |
...
| 17  | `linter-scripts/check-memory-mirror-drift.py`     | Python     | Detects drift between `.ai-memory/memory/index.md` Core section and §X mirror in `21-lovable-folder-structure.md` (presence check on 21 distinctive tokens) | `.ai-memory/memory/index.md`, `02-spec/17-consolidated-guidelines/21-lovable-folder-structure.md` | `0` no drift · `1` drift detected · `2` structural error | `python3 linter-scripts/check-memory-mirror-drift.py`                        |
...
**Resolution rule:** `check-spec-folder-refs.py` exits non-zero with `Stale references found` if a markdown file references a `02-spec/NN-name/` path that:
...
_Validator Inventory added — v3.3.0 — 2026-04-22_
