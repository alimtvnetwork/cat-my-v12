...
Rule of thumb: readers resolve a Project's active rules by walking `ProjectRuleSet -> (snapshot_id or rule_set_id)`, never by dereferencing `rule_set_id` when `override_mode = Snapshot`. This is the same contract as `02-spec/24-app-ui-design-system/22-override-modes.md`.
