...
**Anchor:** `02-spec/21-app/21-root-db.md` (contract) · `02-spec/21-app/05-db-conventions-digest.md` (naming).
**Owns:** `app/core/io/migrations/root/*.sql`.
**Status:** Locked.

## 1. Casing Waiver (overrides `02-spec/04-database-conventions/01`)

This app uses **PascalCase table names, camelCase column names, camelCase JSON keys** — matching `02-spec/21-app/05-db-conventions-digest.md` and `.ai-memory/coding-guidelines.md` §"Data & Schema Rules" rule 2.

Where this conflicts with the generic `02-spec/04-database-conventions/01-naming-conventions.md` "Column names — PascalCase" rule (L21), the folder-level app spec wins per the meta-rule in `01-prompts/86-next-task.md` §"Additional Instruction": _"prefer the folder-level spec … and call out the conflict."_
...
- Documented FK relationships (declared via `001` add-column-less approach: SQLite cannot add FKs to existing tables, so v1 relies on `PRAGMA foreign_keys=ON` + application-level enforcement; a v2 `CREATE TABLE ... FOREIGN KEY` rebuild is tracked in `02-spec/21-app/46-open-questions.md`).
