...
amends: 02-spec/21-app/69-v2-denial-tuning-contract.md §4
---

# 69a - Denial-Burst Tuning Evidence (Provisional)

Numeric evidence for the `derive_denial_defaults` rule in spec 69 §4. Companion to the locked contract at `02-spec/21-app/69-v2-denial-tuning-contract.md`.
...
See `.ai-memory/plans/subtasks/29-denial-burst-threshold-tuning/04-candidate-evaluation.md` for the reproducible synthetic run (seed=29, 174,760 rows, 90-day span).
...
Field replacement: an anonymised 90-day export at `.ai-memory/plans/subtasks/29-denial-burst-threshold-tuning/evidence/*.jsonl` (gitignored) plus a rerun of `evaluate_all` closes the provisional status.
...
- Contract: `02-spec/21-app/69-v2-denial-tuning-contract.md` §4.
- Execution order: `02-spec/21-app/62-v2-execution-order.md` Rank 4 / v2.0.3 row.
- Open question: `02-spec/21-app/shell/24-open-questions.md` SH-Q-07 (kept OPEN until field data lands).
