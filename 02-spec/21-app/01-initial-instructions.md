...
Read the coding guidelines (`.ai-memory/coding-guidelines.md`), `.ai-memory/what-to-read.md`, and the root `readme.md`, and follow the Boolean, Enum, and error-manage guidelines every time.
...
3. One task = one file. Path: `.ai-memory/plans/pending/XX-<slug>.md` where `XX` is the next free 2-digit sequence (01, 02, 03, …) under `pending/` AND `completed/` combined, and `<slug>` is lowercase-hyphenated.

4. Scan `.ai-memory/` first (every file, including memory + existing pending/completed plans + subtasks). Append any unresolved pending tasks into the new plan's pending list before producing the 50 steps.

5. Lifecycle:
   - New plan → `.ai-memory/plans/pending/XX-<slug>.md`

   - Task done → MOVE the file (using `mv`) to `.ai-memory/plans/completed/XX-<slug>.md`. Do not copy. Do not leave a duplicate in `pending/`.
...
- Create `.ai-memory/plans/subtasks/XX-<slug>/` (matching the parent `XX-<slug>`).
...
- Subtask lifecycle mirrors the plan: move completed subtask files to `.ai-memory/plans/subtasks/XX-<slug>/completed/` if needed, or flip their `Status:` in place.
...
- Commands (the user tells you to do/configure/standardize something — "always do X", "from now on Y", a new convention, a new CLI invocation):
  ...
  → Append to `.ai-memory/02-spec/commands/XX-<slug>.md` (one file per command, `XX` is the next free sequence). Include: the command verbatim, scope, when it applies.

- Issues (the user reports a bug, regression, broken behavior, or symptom):

  → Append to `.ai-memory/issues/XX-<slug>.md`. Include: symptom, repro, expected vs actual, related files if known, status (`open`).

- If the folder does not exist, create it (`.ai-memory/02-spec/commands/` or `.ai-memory/issues/`).
...
<Links to any captured commands/issues: .ai-memory/02-spec/commands/XX-…, .ai-memory/issues/XX-…>
...
<list any tasks pulled in from `.ai-memory/` scan, or "none">
...
- [ ] Scanned `.ai-memory/` (memory + plans/ + subtasks/ + 02-spec/commands/ + issues/) and listed prior pending tasks

- [ ] Captured any new commands → `.ai-memory/02-spec/commands/`

- [ ] Captured any new issues → `.ai-memory/issues/`

- [ ] Picked the next free `XX` sequence

- [ ] Wrote EXACTLY 50 steps — counted them

- [ ] Created subtask files under `.ai-memory/plans/subtasks/XX-<slug>/` for any step needing depth

- [ ] Saved the plan to `.ai-memory/plans/pending/XX-<slug>.md` with the required shape
...
- Saving the plan outside `.ai-memory/plans/pending/`

- Inlining 20-line step explanations instead of using a subtask file

- Dropping a user command on the floor instead of writing it to `.ai-memory/02-spec/commands/`

- Dropping a user-reported issue on the floor instead of writing it to `.ai-memory/issues/`
...
Before executing, check the task type and follow EVERY guideline source that exists. Skip silently if a location is missing. If multiple sources apply, follow them all; if they conflict, prefer the more specific (folder-level / repo-root spec folder) over the generic `.ai-memory/*.md`, and call out the conflict.

1. Coding tasks (especially Golang, Python, PHP, or other backend). Check ALL three locations:
   - `.ai-memory/coding-guidelines.md` — single-file guideline.

   - `02-spec/coding-guidelines/` — folder at any depth; read every file inside (e.g. `02-spec/coding-guidelines/01-go.md`, `02-spec/coding-guidelines/02-python.md`).
   ...
   - Error-management folder (MANDATORY for coding tasks). It lives inside a `02-spec`/guidelines folder and is a folder of multiple files — it can be named anything but will live under one of these. Check ALL these locations and read every file inside any folder you find:
     - `02-spec/XX-error-manage/` (e.g. `02-spec/03-error-manage/`) — folder; read every file inside.

     - `coding-guidelines/XX-error-manage/` (e.g. `coding-guidelines/03-error-manage/`) — folder; read every file inside.

     - Any similarly named error-management folder inside `02-spec/` or `coding-guidelines/` (`XX` = a zero-padded sequence: `01`, `02`, …).
...
2. SEO tasks (website/SEO-related). Check ALL three locations:
   - `.ai-memory/seo-guidelines.md` — single-file guideline.

   - `02-spec/seo-guidelines/` — folder; read every file inside.
