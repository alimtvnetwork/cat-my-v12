...
Status: draft (v0.1)
Owner: BE / CLI
Depends on: `40-error-manage.md`, `41-logging.md`, `42-observability.md`, `02-spec/03-error-manage/**`, `02-spec/05-split-db-architecture/**`, `02-spec/06-seedable-config-architecture/**`, `02-spec/04-database-conventions/**`
...
2. Field names PascalCase (matches Universal Envelope convention from `02-spec/03-error-manage/`).
...
Main app reads `index/current.json` to enumerate live and recent sessions, then tails `LogPath` for detail. Writes go through a file lock (`index/current.json.lock`) per `02-spec/13-generic-cli/16-verbose-logging.md`.
...
Per `02-spec/05-split-db-architecture/`:
...
Migration files follow `02-spec/04-database-conventions/` and `02-spec/21-app/26-migrations.md`.

## Seedable config

Per `02-spec/06-seedable-config-architecture/`, both CLIs load config in this layer order: defaults -> repo `config/*.toml` -> `<APP_CONFIG_ROOT>/*.toml` -> env vars -> CLI flags.
...
- `02-spec/03-error-manage/` registry extension
