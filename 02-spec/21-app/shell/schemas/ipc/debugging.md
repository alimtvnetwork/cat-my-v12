...
- Reproduce the failure or capture the failing log line first. If there are
  ...
  `02-spec/21-app/41-logging.md`; do not swallow errors).
- Keep the failing correlation_id handy: every step below filters by it.

## 1. Read the error log line

Every IPC error must carry these fields (see `02-spec/21-app/40-error-manage.md`
Appendix A and `02-spec/21-app/41-logging.md`):
...
`02-spec/21-app/shell/schemas/ipc/readme.md` "Group files" table. Example:
...
- Rule: every `*.req` / `*.res` name referenced in
  `02-spec/21-app/shell/05-ui-to-backend-map.md` MUST have a `$defs` entry.
...
- **Wrong type / enum:** compare against `type` / `enum` / `pattern` on the
  node. Prefer PascalCase enum values (see `02-spec/21-app/33-rule-catalog.md`
  reason-code table).
...
- Passing payload → `linter-scripts/fixtures/ipc-examples/good/02-spec/21-app/shell/schemas/ipc/examples/<group>/<method>.<direction>.json`
...
- Amend the schema `$defs` entry AND add a `good/` fixture AND bump the
  ...
  `02-spec/21-app/shell/schemas/ipc/readme.md` if the method list changed.
...
error-management contract in `02-spec/21-app/40-error-manage.md`.
...
`02-spec/21-app/40-error-manage.md` Appendix A in the same commit. Unregistered
...
| -------------------------- | ---------------------------------------------------- | ------------------------------------------------------------------------- |
| `E_SPEC_UI_MAP_ORPHAN`     | method in map without `$defs` (or vice versa)        | `02-spec/21-app/shell/05-ui-to-backend-map.md` and the group `*.schema.json` |
| `E_IPC_SCHEMA_MISMATCH`    | payload shape ≠ `$defs` entry                        | group `*.schema.json` at `$defs["<method>.<direction>"]`                  |
| `E_INSTRUCTION_SCHEMA`     | canonical JSON instruction doesn't match v2 envelope | `02-spec/21-app/36-json-instruction-output.md` + `instruction.schema.json`   |
| `E_IPC_UNKNOWN_METHOD`     | caller uses a name absent from the map               | `02-spec/21-app/shell/05-ui-to-backend-map.md`                               |
