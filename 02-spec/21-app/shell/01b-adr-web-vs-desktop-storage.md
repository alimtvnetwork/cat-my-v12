...
`02-spec/23-app-db/*`, `02-spec/05-split-db-architecture/*`, and the
...
(`02-spec/23-app-db/01-root-db-schema.md`, `02-spec/05-split-db-architecture/*`)
...
- **Hybrid, Postgres canonical + SQLite mirror.** Web is the primary
  ...
  (`02-spec/21-app/shell/15-data-migration.md`, Plan 64 steps 86-88:
...
- The SQLite schema in `02-spec/23-app-db/*` is now labelled "bundle
...
- Split-DB layering (`02-spec/05-split-db-architecture/*`) applies to the
...
- `02-spec/23-app-db/01-root-db-schema.md` §4 stays valid as the bundle
...
- `02-spec/21-app/shell/15-data-migration.md`: bundle import / export.
- `02-spec/23-app-db/01-root-db-schema.md`: bundle-format DDL.
- `02-spec/05-split-db-architecture/*`: applies to bundle only.
