...
A "spec change" is any edit under `spec/**` or any code/memory change that alters a contract declared in `02-spec/21-app/*`. Cosmetic edits (typos, formatting) are still recorded but do not bump the minor version.
...
The `next task` alias in `.ai-memory/prompt.md` MUST advance to the newly archived prompt file in the same commit as the version bump. Stale alias is `E_PROMPT_ALIAS_STALE`.
