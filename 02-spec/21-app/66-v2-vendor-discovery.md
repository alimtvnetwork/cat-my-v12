...
The wire-level request/response shapes, Zod validators, HTTP codes, and failure envelope live in `02-spec/21-app/67-v2-discovery-contract.md` (single source of truth). This section pins only what the module contract in §Module contract owes to that wire contract:
...
- All three server-fn failure codes (`E_CFG_UNKNOWN_DEVICE`, `E_SEC_ROLE_DENIED`, `E_SEC_RATE_LIMITED`) are registered in `02-spec/21-app/40-error-manage.md`; no ad-hoc string codes may leak to the client envelope.
...
The facade is the only module allowed to aggregate vendor descriptors. It returns the Zod shape from `02-spec/21-app/67-v2-discovery-contract.md` and never returns raw SDK descriptor objects.

## Contract back-links

| Target                                     | Required use                                                                                          |
| ------------------------------------------ | ----------------------------------------------------------------------------------------------------- |
| `02-spec/21-app/52-sdk-facade-pattern.md`     | `VendorDiscoveryFacade`, `CatDeviceDescriptor`, and no raw SDK descriptor leaks past the facade seam. |
| `02-spec/21-app/67-v2-discovery-contract.md`  | `getDiscoveredDevices`, `selectCaptureDevice`, Zod schemas, and unknown-selection behavior.           |
| `02-spec/21-app/68-v2-vendor-sdk-contract.md` | Shared vendor names, lifecycle selection, and `CatDeviceDescriptor` vocabulary.                       |
| `02-spec/coding-guidelines/python.md`         | Runtime SDK guards and descriptor logging.                                                            |
| `02-spec/coding-guidelines/typescript.md`     | Server function validation and UI correlation ids.                                                    |
