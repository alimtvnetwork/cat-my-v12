...
Related: `02-spec/13-generic-cli/**`, `02-spec/03-error-manage/**`, `02-spec/05-split-db-architecture/**`, `02-spec/06-seedable-config-architecture/**`
...
- Reads seedable config per `02-spec/06-seedable-config-architecture/` (device profile, exposure/gain defaults, trigger mode, output dir).
...
- Persists device+capture events into the Root DB per `02-spec/05-split-db-architecture/`; per-run capture metadata into the Task DB.
- Emits Universal Envelope on stdout when `--json` set; human-readable table otherwise (per `02-spec/13-generic-cli/06-output-formatting.md`).
- Error codes limited to registered `E_CAM_*`, `E_BE_*`, `E_CLI_*` families (see `02-spec/03-error-manage/`).
...
1. `worker-cli --help` prints subcommand tree matching `02-spec/13-generic-cli/09-help-system.md`.
...
7. Followed guidelines: `02-spec/02-coding-guidelines/`, `.ai-memory/coding-guidelines.md`. No magic strings. All identifiers PascalCase where wire-facing.
...
- Config: `config/worker.toml` (path overridable by `--config`) layered per `02-spec/06-seedable-config-architecture/`.
