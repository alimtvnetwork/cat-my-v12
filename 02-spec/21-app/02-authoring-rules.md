...
> **Source of truth:** [`.ai-memory/coding-guidelines.md`](../../.ai-memory/coding-guidelines.md)
> **Scope:** Rules every file under `02-spec/21-app/**` must follow. These are the guardrails future code will be measured against, so specs must match them exactly.
...
- Every DB spec MUST include a **Mermaid ERD** and cite `02-spec/04-database-conventions/` + `02-spec/05-split-db-architecture/`.
...
Every leaf spec file under `02-spec/21-app/` MUST have:
...
Folder-level spec (`spec/**/error-manage/`, `02-spec/05-split-db-architecture/`, `02-spec/04-database-conventions/`) beats a local rule. Call out the conflict in the affected file's `## Cross-References` block.

## Acceptance Checklist

- [ ] Every new spec under `02-spec/21-app/` conforms to the naming/charset rules stated here or emits `E_NAME_INVALID_CHARSET`.
- [ ] PascalCase enum policy in this file matches `.ai-memory/memory/09-enums-and-results-shape.md` verbatim; drift = `E_SPEC_TAXONOMY_CONTRADICTION`.
- [ ] Coding-guideline pointer resolves to `02-spec/coding-guidelines/{python,typescript,sql}.md`; missing folder = `E_SPEC_GUIDELINE_MISSING`.
