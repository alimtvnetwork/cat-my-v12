...
Owner: app team
Created: 2026-07-21
Related command: `.ai-memory/02-spec/commands/40-backend-mode-toggle-and-sdk-facade.md`
Related plan: `.ai-memory/plans/pending/88-backend-implementation-v1-150-steps.md`
Related ADR: `docs/adr/0001-backend-mode-and-facade.md`
Gap log driving this revision: `docs/plans/88/spec-gaps.md`

## Purpose

Introduce a real backend (`BE/` folder at repo root) that the current frontend can talk to, while preserving the existing seeded UI mode for offline / demo use. Enforce a facade layer between the app and the (future) camera SDK on BOTH frontend and backend. Wire full error management per `02-spec/21-app/40-error-manage.md` and the incoming `02-spec/21-app/40b-http-envelope.md` sub-spec.
...
5. Error management (`02-spec/21-app/40-error-manage.md` + `40b-http-envelope.md`) is mandatory:
...
6. Launcher: `run.ps1` (Windows) and `run.sh` (POSIX) at repo root start backend + frontend together per new `02-spec/21-app/shell/26-dev-launcher.md`. Env contract: `BE_PORT` (default 8787), `FE_PORT` (default 5173), `UI_BACKEND_BASE_URL`. Chromium extension shell scope pending re-read of `shell/01-adr-shell-choice.md`; if that ADR blocks a production extension shell, `chromium-shell/` is downgraded to a dev-only harness and the request is amended.
7. Coding guidelines: strict TS, no `any`, PascalCase JSON keys, enum types with `Type` suffix, no magic strings, function bodies <= 15 lines, no nested `if`. See `.ai-memory/memory/24-coding-and-error-rulebook.md` and `docs/plans/88/guideline-digest.md`.
...
- New locked specs: `02-spec/21-app/40b-http-envelope.md`, `02-spec/21-app/shell/26-dev-launcher.md`; patches to `27-config-surface.md`, `20-folder-structure.md`, `30-ui-overview.md`, `39-settings-screen.md`, `41-logging.md`, `44-security-privacy.md`.
