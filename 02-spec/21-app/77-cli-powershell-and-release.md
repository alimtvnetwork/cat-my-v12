...
Status: draft (v0.1)
Owner: DevEx / CI
Depends on: `74-worker-cli.md`, `75-processing-cli.md`, `76-cli-log-and-ipc.md`, `02-spec/11-powershell-integration/**`, `02-spec/12-cicd-pipeline-workflows/**`, `02-spec/16-generic-release/**`

## Intent

Provide PowerShell wrappers to run each CLI locally on Windows AND ship a GitHub release with a one-line PowerShell installer that fetches, verifies, and installs both CLIs on an end-user machine. All CI/CD strictly follows `02-spec/12-cicd-pipeline-workflows/` and `02-spec/16-generic-release/`.
...
- All comply with `02-spec/11-powershell-integration/03-integration-guide.md` (parameter naming, error record shape, `[CmdletBinding()]`, `-WhatIf` for state-changing subcommands).

### Release artefacts

Per `02-spec/16-generic-release/05-release-assets.md`:
...
- `SHA256SUMS.txt`, `SHA256SUMS.txt.asc` (optional sign, per `02-spec/12/05-code-signing.md`)
- `install.ps1` (PowerShell installer script for Windows)
- `install.sh` (bash installer for Linux, per `02-spec/16-generic-release/03-install-scripts.md`)
- `release-notes.md` generated from CHANGELOG segment for this version

### PowerShell installer (`install.ps1`)

Per `02-spec/12-cicd-pipeline-workflows/04-install-script-generation.md` and `02-spec/16-generic-release/03-install-scripts.md`:
...
Per `02-spec/12-cicd-pipeline-workflows/`:
...
- Guards per `02-spec/12/03-reusable-ci-guards/`.
- Vulnerability scan per `02-spec/12/03-vulnerability-scanning.md` runs pre-build; blocks on Critical.
...
6. Release notes list every merged PR since last tag (per `02-spec/12/07-release-body-and-changelog.md`).
