...
Locks the exact request/response shape, validation rules, and failure modes for the two v2.0.1 server functions exposed to the Settings UI. Companion to `02-spec/21-app/66-v2-vendor-discovery.md` (module contract) and `02-spec/21-app/62-v2-execution-order.md` (exit criteria E1..E6).
...
| ----------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| 02-spec/21-app/50-capture-modules.md §VendorDeviceIO contract lock                     | `VendorDeviceDescriptor` shape, close ordering, `E_CAP_*` codes                                                                                                                                                 | Plan 25 SS-02 lock     |
| 02-spec/21-app/52-sdk-facade-pattern.md                                                | SDK Facade Pattern; `E_BUG_SDK_LEAK` lint gate                                                                                                                                                                  | §2 Enforcement         |
| 02-spec/21-app/63-v2-vendor-pylon.md, 64-v2-vendor-spinnaker.md, 65-v2-vendor-vimba.md | `list_devices` + normalized error mapping per vendor                                                                                                                                                            | Vendor facade sections |
| 02-spec/21-app/66-v2-vendor-discovery.md                                               | Aggregation semantics, `W_DISCOVERY_PARTIAL` warning                                                                                                                                                            | §Aggregation           |
| 02-spec/21-app/40-error-manage.md                                                      | Wire codes: `E_SEC_UNAUTH`, `E_SEC_DENIED`, `E_CFG_BAD_INPUT`, `E_CFG_UNKNOWN_DEVICE`, `E_CFG_PERSIST_FAILED`, `E_SEC_AUDIT_FAILED`, `E_CAP_SDK_ABSENT`, `E_CAP_ENUM_FAILED`, `E_INTERNAL`, `I_SEC_ADMIN_WRITE` | Appendix A             |
| 02-spec/21-app/72-audit-persistence.md                                                 | Durable sink for `I_SEC_ADMIN_WRITE` on subject `settings.capture.device`                                                                                                                                       | §Codes                 |
