...
Version: v2.76.0
Date: 2026-07-14
Scope: `02-spec/21-app/` (61 files)
Rubric: see `.ai-memory/memory/v2/plan23/13-paths-and-rescore.md` (5 checks, 20 pts each).
...
Concrete codes in these families are registered in `02-spec/21-app/40-error-manage.md` Appendix A per Step 21. Waived: prefix templates are documentation, not emitted codes.
...
Cleared: 67 gained `## Facade Binding` (binds `selectCaptureDevice` / `getDiscoveredDevices` / `SettingsStore.write_capture_device` to `VendorDeviceIO` per spec 52) and `## Contract back-links` (50 / 52 / 63-66 / 40 / 72). Full artifact: `02-spec/25-app-audit/latest/plan25-ss09/00-rescore.md` + `rescore.json`. Rerun: `python3 scripts/rescore_plan25_ss09.py`.

## Source

- Rescore script + transcript: `.ai-memory/memory/v2/plan23/13-paths-and-rescore.md`.
- Consolidated bundle: `02-spec/25-app-audit/latest/99-consolidated.md`.
- Remediation trace: `02-spec/25-app-audit/93-remediation-evidence.md`.
- Plan 25 SS-09 subset: `02-spec/25-app-audit/latest/plan25-ss09/`.
