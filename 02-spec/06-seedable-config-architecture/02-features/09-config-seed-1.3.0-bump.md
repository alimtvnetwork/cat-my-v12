...
**Resolves:** FU-12 from `02-spec/19-main-worker-service/14-rbac-and-status-seed.md` §1.
...
2. New `Categories.MainWorker` category carrying the 27 tunables from `02-spec/19-main-worker-service/15-tunable-constants.md`.
...
| `Categories.MainWorker.*`    | add (27 keys)               | `02-spec/19/15-tunable-constants.md` §3    | none — new category   |
| `Tables.Role`                | add (3 rows)                | `02-spec/19/14-rbac-and-status-seed.md` §3 | none — new table seed |
...
```jsonc
{
  ...
  "Tables": {
    "Role": {/* see 02-spec/19/14 §3 */},
    "EnumPage": {/* see 02-spec/19/14 §3 */},
    "RolePageAccess": {/* see 02-spec/19/14 §3 */},
    "WorkerNodeStatus": {/* see 02-spec/19/14 §3 */},
    "AuthMechanism": {/* see 02-spec/19/14 §3 */},
  },
}
```

Concrete row sets are not duplicated here — `02-spec/19/14-rbac-and-status-seed.md` §3 is the single source of truth.

---

## 4. New `Categories.MainWorker` block

Verbatim from `02-spec/19-main-worker-service/15-tunable-constants.md` §3:
...
"MainWorker": {
  ...
  /* … balance of the 27 keys per 02-spec/19/15 §3 … */
}
```

Implementer note: the 18 omitted keys live in `02-spec/19/15` §3; copy them verbatim. They are not re-listed here to keep this file the **delta** rather than a duplicate.
...
Mirrors `02-spec/19/14-rbac-and-status-seed.md` §4 — must all pass:
...
- `02-spec/06-seedable-config-architecture/01-fundamentals.md` — `SeedWithVersionCheck` algorithm.
- `02-spec/06-seedable-config-architecture/02-features/07-reference-table-seeding.md` — `Tables` block mechanism.
- `02-spec/06-seedable-config-architecture/02-features/08-config-schema-tables-block.md` — JSON-Schema fragment used at lint time (FU-11).
- `02-spec/06-seedable-config-architecture/02-features/10-at-ref-resolver.md` — `@-ref` resolver (FU-13).
- `02-spec/19-main-worker-service/14-rbac-and-status-seed.md` — concrete row sets.
- `02-spec/19-main-worker-service/15-tunable-constants.md` §3 — concrete `MainWorker` category values.
...
_config.seed.json bump 1.3.0 v1.0.0 — 2026-05-04 (FU-12 closed)_
