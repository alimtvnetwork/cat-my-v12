...
**Authority:** Extends the `config.seed.json` schema defined in `02-spec/06-seedable-config-architecture/01-fundamentals.md` with a sibling top-level `Tables` block for seeding small reference / enum / join tables. Existing `Categories` block is unchanged.

---

## 1. Why this feature exists

`config.seed.json` (per `01-fundamentals.md`) seeds **key/value settings** stored in the `config` table. Several specs — including `02-spec/19-main-worker-service/07-role-based-dashboards.md` — also need to seed **reference tables** (e.g. `Role`, `EnumPage`, `RolePageAccess`, `WorkerNodeStatus`) at install time and merge new rows on SemVer bumps.
...
```jsonc
{
  ...
  "Tables": {
    "<TableName>": {
      ...
      "Rows": [{ "<ColumnName>": "<Value>", "Description": "..." }],
    },
  },
}
```
...
| `ReplaceAll` _(reserved)_ | Truncate table then INSERT all rows. **Not implemented in v1.0** — destructive.                    |
...
Per `02-spec/19-main-worker-service/07-role-based-dashboards.md` and `10-worker-bootstrap-protocol.md`, the following Main-tier tables MUST be seeded via this mechanism. Concrete row sets live in `02-spec/19-main-worker-service/14-rbac-and-status-seed.md`.
...
- `02-spec/06-seedable-config-architecture/01-fundamentals.md` — sibling `Categories` block; merge-on-version-bump pattern.
...
- `02-spec/19-main-worker-service/07-role-based-dashboards.md` — RBAC schema that consumes this feature.
- `02-spec/19-main-worker-service/10-worker-bootstrap-protocol.md` §8 — `WorkerNodeStatus` rows.
- `02-spec/19-main-worker-service/14-rbac-and-status-seed.md` — concrete row-set binding.

---

_Reference-table seeding v1.0.0 — 2026-05-04_
