...
**Resolves:** FU-11 from `02-spec/19-main-worker-service/14-rbac-and-status-seed.md` §3.1.
...
```jsonc
{
  ...
  "$defs": {
    ...
    "AtRefString": {
      ...
      "description": "Logical reference; resolved at apply time per 02-spec/06/02-features/10-at-ref-resolver.md.",
    },
...
Valid — `EnumPage` block from `02-spec/19/14-rbac-and-status-seed.md` §3:
...
```
ajv validate \
  -s 02-spec/06-seedable-config-architecture/02-features/08-config-schema-tables-block.md#section-2 \
  -d 02-spec/19-main-worker-service/14-rbac-and-status-seed.md#section-3
```
...
- `02-spec/06-seedable-config-architecture/01-fundamentals.md` — sibling `Categories` block.
- `02-spec/06-seedable-config-architecture/02-features/07-reference-table-seeding.md` — feature definition this file mechanizes.
- `02-spec/06-seedable-config-architecture/02-features/09-config-seed-1.3.0-bump.md` — first seed file that uses this schema (FU-12).
- `02-spec/06-seedable-config-architecture/02-features/10-at-ref-resolver.md` — `@<Table>.<Code>` resolver (FU-13).
- `02-spec/19-main-worker-service/14-rbac-and-status-seed.md` — first concrete consumer.
...
_Tables-block schema v1.0.0 — 2026-05-04 (FU-11 closed)_
