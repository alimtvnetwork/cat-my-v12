...
Stored in `shapes` table (see `02-spec/23-app-db/05-user-assets.mmd`). After compile the user is returned to the standard editor with the new shape selected and referenced by `region_shape_id` on the active rule.
