...
Screen: Hmi add tools ribbon marking overview
Related-Spec: 03-canvas.md
---

# 02 � Hmi add tools ribbon marking overview

## 1. One-line purpose

This screen is the primary interface for Hmi add tools ribbon marking overview, allowing operators to configure, inspect, or manage related settings.
...
- "Hmi add tools ribbon marking overview"
...
1. Button (Text: "Hmi add tools ribbon marking overview"): Top left title area, expected to show a dropdown menu when clicked. Enabled.
...
The operator is currently on the Hmi add tools ribbon marking overview screen. They likely just navigated from the main menu or a previous tool configuration screen. Their immediate goal is to review or adjust the settings specific to Hmi add tools ribbon marking overview. After completing the setup, they will likely click Apply/Save and return to the main run screen or proceed to the next tool in the sequence.
...
- **Primitives mapping:** This screen configures a rule of kind matching `Hmi add tools ribbon marking overview` logic. ROIs are standard rectangular or polygonal shapes.
