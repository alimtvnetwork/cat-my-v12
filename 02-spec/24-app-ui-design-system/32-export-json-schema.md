...
**Depends on:** `15-export-import.md`, `02-spec/23-app-db/01-root-db-schema.md` §4.
...
- `checksumSha256` is computed over `JSON.stringify(payload, sortedKeysReplacer)` with UTF-8 encoding, matching `computeSha256` in `02-spec/23-app-db/01-root-db-schema.md`.
