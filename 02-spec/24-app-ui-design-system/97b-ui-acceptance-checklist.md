...
**Domain source of truth:** `02-spec/21-app/` (initial instructions, screens, shape model, rule catalog, tolerances, zoom/pan, JSON instruction output, error/logging).
**Editor shell source of truth:** `02-spec/24-app-ui-design-system/` (tokens, layout, canvas, rail, controller, state, errors, testing).
...
Plan 64 (`.ai-memory/plans/pending/64-plan24-ui-v2-recipes-rules-and-desktop-overhaul.md`) has landed the following steps in code. Individual gates in §1-§8 below are still the authoritative ticks; this section is a quick locator so reviewers can find where each step lives.
...
- `02-spec/21-app/01-initial-instructions.md` (system intent)
- `02-spec/21-app/30-ui-overview.md` through `39-settings-screen.md` (screens)
- `02-spec/21-app/32-shape-model.md`, `33-rule-catalog.md`, `34-tolerance-model.md`, `35-zoom-and-pan.md`, `36-json-instruction-output.md`
- `02-spec/21-app/40-error-manage.md` (App A code registry)
- `02-spec/21-app/41-logging.md` (wire format)
- `02-spec/24-app-ui-design-system/00-overview.md` through `08-testing.md`
- `02-spec/24-app-ui-design-system/97-acceptance-criteria.md` and this file
- `.ai-memory/plans/subtasks/30-app-ui-rule-editor-revamp/SS-01..SS-05` (all five)
...
1. `[ ]` DEC-01..DEC-08 decision notes under `02-spec/24-app-ui-design-system/_notes/`.
...
6. `[ ]` Reconcile every new `I_UI_*/W_UI_*/E_UI_*` code with `02-spec/21-app/40-error-manage.md` App A per DEC-07.
...
- **P64-DOM-02** `[ ]` Storage columns and API function names use `RuleSet` / `Rule`. (`02-spec/23-app-db/01-root-db-schema.md` §4, `20-backend-endpoint-map.md`.)
