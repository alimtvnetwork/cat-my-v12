...
9. Follow `.ai-memory/coding-guidelines.md` and `02-spec/coding-guideline/` when present.

## Plan

### Phase 1: Next 10 Steps (What To Do)

1. Read `.ai-memory/coding-guidelines.md`, `.ai-memory/what-to-read.md`, root `README.md`, spec folders 21 (architecture), 23 (diagrams), and existing 24 content.
2. Create spec folder `02-spec/24-vision-inspection-ui/` with subfolders `app-db/` (Mermaid + PNG), `flows/`, `panels/`, `endpoints/`.
...
9. Define ambiguity capture protocol: every unclear point becomes a numbered entry in `.ai-memory/ambiguous-questions.md` with context, options, and blocking/non-blocking flag.
...
32. Draw ERD Mermaid in `02-spec/24-vision-inspection-ui/app-db/erd.mmd` + PNG render.
...
40. Define ambiguity list template and seed `.ai-memory/ambiguous-questions.md`.
...
100.  Final ambiguity review pass: resolve or flag as blocking in `.ai-memory/ambiguous-questions.md`.
...
## Ambiguity Questions (Seed for `.ai-memory/ambiguous-questions.md`)
...
Read `.ai-memory/coding-guidelines.md`, `.ai-memory/what-to-read.md`, root `README.md`, and any `/02-spec/coding-guideline/` and `/02-spec/error-manage/` folders. Follow Boolean, Enum, and error-management guidelines. Every catch must be logged. No `any`, `unknown`, or wide types. Files under 80-100 lines. Booleans prefixed with `is`/`has`. Variables assigned once (Rust-style). Assets under `/assets/xx-folder-name/xx-file-name.ext`.
...
7. Every ambiguity is captured in `.ai-memory/ambiguous-questions.md` with context and blocking flag.
...
4. Scan `.ai-memory/` first (every file, including memory + existing pending/completed plans + subtasks). Append any unresolved pending tasks into the new plan's pending list before producing the 100 steps.
