...
Ran `rg -n "TODO|TBD|FIXME|XXX" 02-spec/24-app-ui-design-system/` against the
...
Total hits in `02-spec/24-app-ui-design-system/`: **1**.
...
- `.ai-memory/plans/subtasks/30-app-ui-rule-editor-revamp/`: contains `TODO`
  markers inside plan step bodies. These are the plan's own working notes,
  not spec debt, and are removed as each step closes.
- `.ai-memory/memory/**`: excluded from spec sweep; memory files are living
  notes.
...
```bash
rg -n "TODO|TBD|FIXME|XXX" 02-spec/24-app-ui-design-system/ \
