...
- `97b-ui-acceptance-checklist.md` — executable to-do form of the acceptance gates, grounded in a full read of `02-spec/21-app/01-initial-instructions.md` and every §30-41 file. Every checkbox cites the spec line(s) it enforces.
- §1 Contradiction resolution DEC-01..DEC-08 records the eight cross-spec conflicts that must be decided before any UI code:
  ...
  - DEC-07 Error code namespace: every new `I_UI_*/W_UI_*/E_UI_*` reconciled against `02-spec/21-app/40-error-manage.md` App A.
...
- Namespace cross-link appended to `02-spec/03-error-manage/03-error-code-registry/01-registry.md` clarifying that `I_UI_*` / `W_UI_*` / `E_UI_*` / `I_CAM_*` are structured log events (spec 24 is source of truth) and must be ignored by the numeric-code linter. (project v3.26.0)

### Compliance

- Spec-authoring-guide requirement: every spec folder MUST carry its own `98-changelog.md`. Satisfied at project v3.28.0.
- `02-spec/spec-index.md` updated at project v3.27.0: App UI count 1 → 9, total 411 → 419.
...
- `../../.ai-memory/plans/subtasks/30-app-ui-rule-editor-revamp/ss-03-math-expression-grammar.md` locks the safe Math rule grammar, 10 evaluator vectors, 8 geometry vectors, parser failure reasons, and test ownership for `math-evaluator.test.ts`, `coords.test.ts`, and `hit-test.test.ts`.
- `../../.ai-memory/plans/subtasks/30-app-ui-rule-editor-revamp/ss-05-lighting-controls.md` locks 7 lighting controls, ranges, defaults, validation rules, 4 log codes, 12 LC acceptance rows, and `lighting.test.ts` / `lighting.spec.ts` coverage.
...
- `_notes/todo-tbd-sweep.md` added: swept `02-spec/24-app-ui-design-system/` for `TODO|TBD|FIXME|XXX`, 1 hit found (meta-marker in this changelog), zero rewrites required, regression guard defined for step 35.
...
## Upcoming (unreleased, pinned in `01-prompts/351-next-task.md`)
...
- DB diagrams: `02-spec/23-app-db/02-rule-sets.mmd`, `03-projects.mmd`, `04-runs-captures.mmd`, `05-user-assets.mmd`. Root DB schema gained §4 SQLite Column Contract.
...
Q1, Q4, Q6-Q12, Q15-Q21 remain open. Answers propagate through `.ai-memory/ambiguity-questions/00-index-blocked-specs.md`.
