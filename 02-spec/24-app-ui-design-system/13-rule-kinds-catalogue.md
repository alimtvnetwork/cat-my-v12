...
Enumerate every rule kind the editor exposes in v1, with its storage token, UI label, parameter set (DB columns), and validator contract. Any new kind must be added here first and to the SQLite column contract in `02-spec/23-app-db/01-root-db-schema.md` in the same change.
