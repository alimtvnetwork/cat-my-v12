...
**Depends on:** `02-spec/23-app-db/05-user-assets.mmd`, endpoint row 13, `20-backend-endpoint-map.md`.
...
- Table: `JsFunction` + `JsFunctionVersion` per `02-spec/23-app-db/05-user-assets.mmd`.
...
- Provided modules via `ctx`: `logger` (proxy to server log), `now()`, `sha256(bytes)`, `cv` (curated OpenCV.js: `threshold`, `findContours`, `Canny`, `matchTemplate`, `cvtColor`, `resize`). Full API in `02-spec/24-app-ui-design-system/27a-js-cv-api.md` (to be written when Q8 resolves).
