...
Version: 3.0
Supersedes: `02-spec/24-app-ui-design-system/09-UI-improvements-v2.md` (kept for history)
Owners: Vision Inspection UI, Rules Editor, Projects, Run
Related: `02-spec/21-app/`, `02-spec/23-app-db/`, `02-spec/24-app-ui-design-system/*`, `.ai-memory/plans/`
...
8. All new code follows `coding-guidelines/`, `02-spec/02-coding-guidelines/`, `02-spec/03-error-manage/`. Every state change emits a log per the error-management folder.
9. CI/CD runs lint, typecheck, unit, and e2e on every PR. See `.ai-memory/02-spec/commands/24-cicd-lint-integration.md`.
...
| ----- | -------------------------------------------------------------------- | ------- | --------------------------------------------------------------------------- |
| SH-01 | Single global header, no duplicates                                  | PARTIAL | `.ai-memory/issues/22-duplicate-header-still-present.md`; plan 65 pending.    |
...
| SH-08 | Menu hover jitter fixed                                              | DONE    | `.ai-memory/issues/17-menu-hover-jitter-and-padding.md` closed by plan 62/63. |
...
| PR-02 | Project create flow, no dead ends                                      | PARTIAL  | `.ai-memory/issues/16-project-section-create-flow-broken.md` open for edge cases. |
...
| PS-04 | ERD Mermaid + PNG in `02-spec/23-app-db/`                                    | PARTIAL  | Files exist but not regenerated for v3 tables.                    |
...
| CX-03 | CI/CD lint + typecheck + unit + e2e per PR                    | PARTIAL | Scripts exist; workflow needs a single pipeline entry. See `.ai-memory/02-spec/commands/24-cicd-lint-integration.md`. |
...
Every open ambiguity is tracked in `.ai-memory/ambiguity-questions/02-ui-v3-open-questions.md`. Blocking ambiguities halt the corresponding row in section 3 until answered.

## 5. Plan Reference

Level-2 execution plan for the missing rows: `.ai-memory/plans/pending/66-ui-v3-missing-completion.md` (30 steps).
