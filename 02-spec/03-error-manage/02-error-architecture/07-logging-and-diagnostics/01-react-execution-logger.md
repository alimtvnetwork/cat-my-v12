...
captureError: (input) => {
  ...
  const executionChain = logger.enabled ? logger.getFormattedChain() : undefined;
...
// In GlobalErrorModal.tsx - Stack Tab - Frontend Sub-tab
{
  error.executionChain ? (
    <div className="space-y-2">
      ...
      <pre className="bg-muted p-3 rounded text-xs">{error.executionChain}</pre>
    </div>
  ) : (
...
_Specification created: 2026-02-06_
