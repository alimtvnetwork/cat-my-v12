...
| `CAT`   | Control Automation UI structured codes | String codes (`E_LAYER_*`, `W_LAYER_*`, `I_UI_*`) | `02-spec/21-app/40-error-manage.md`, `02-spec/24-app-ui-design-system/07-errors-logging.md` | ✅ Active                                       |
...
| ------------------------------- | ------- | ----------- | ------------------ | ------------------------------------------------------------------- |
| Backend Core                    | `BE`    | 4000-4999   | N/A                | `02-spec/03-error-manage/03-error-code-registry.md`                    |
...
| Code               | Name | Message                 |
| ------------------ | ---- | ----------------------- |
| _(none allocated)_ | —    | Reserved for future use |
...
The ecosystem uses **three** error code formats:
...
| Structured string | Control Automation app UI and observability | `E_LAYER_REORDER_FAILED` | `^[EWI]_[A-Z0-9_]+$`             |

All three formats are valid. Go CLI tools use flat integers in API responses, constants, and logs. PHP plugins and general specs use the prefixed format. Control Automation app UI uses structured string codes because these are log/audit and user-surface identifiers, not ecosystem numeric API codes. The JSON schema supports the numeric and prefixed formats; structured string codes are indexed by their owning app spec.
...
## Plan 30 / 35 - App UI (rule editor) log codes

Structured log codes for the rule editor (Setup / ROI / Reference) live under a different naming scheme (`I_UI_*`, `W_UI_*`, `E_UI_*`, `I_CAM_*`) because they identify **log events**, not user-visible domain errors, and are namespaced by subsystem (`UI`, `CAM`). They are documented in full — with keys, triggers, and UI actions — in [`02-spec/24-app-ui-design-system/07-errors-logging.md`](../../24-app-ui-design-system/07-errors-logging.md).

Plan 35 adds layer interaction codes to the same structured-code family. The canonical rows live in `02-spec/21-app/40-error-manage.md` Appendix A.4:

| Code                         | Tier               | Surface                                                        | Context schema                                       |
| ---------------------------- | ------------------ | -------------------------------------------------------------- | ---------------------------------------------------- |
| `E_LAYER_REORDER_FAILED`     | `BugError`         | Layer drag/drop or keyboard reorder failure.                   | `subject=sourceId`, `targetId`, `position`, `reason` |
| `E_LAYER_MERGE_INCOMPATIBLE` | `DomainError`      | Merge command rejected because the selection cannot be merged. | `subject=selectionKey`, `reason`, `count`, `kinds`   |
| `W_LAYER_GROUP_EMPTY`        | Observability warn | Empty or stale group pruned during group, ungroup, or delete.  | `subject=groupId`, `trigger`, `childCount=0`         |

Rules:

- Do NOT allocate a numeric range here for these codes; they are not user-facing error codes.
- Domain errors that surface to the user (e.g. persistence failures) still map through the editor's `E_UI_RULE_SAVE_FAILED` / `E_UI_CANVAS_LOAD` etc.; if any of those need a user-facing numeric code as well, register the numeric code here in the standard range and reference the log code in its message.
- The 08-linter-scripts must ignore `I_UI_*`, `W_UI_*`, `E_UI_*`, `I_CAM_*` when scanning source for un-registered numeric codes.
