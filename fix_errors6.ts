...
walk('src/routes', (p) => {
    ...
    if (p.includes('settings.trigger.tsx')) {
        ...
        content = content.replace(/import { TriggerTimingDiagramPropsEdgeType } from '..\/..\/components\/settings\/TriggerTimingDiagram';\n/g, '');
        content = content.replace(/type TriggerEdgeType = TriggerTimingDiagramPropsEdgeType;/g, "import { TriggerTimingDiagramPropsEdgeType } from '@/components/settings/TriggerTimingDiagram';");
        content = content.replace(/import { TriggerTimingDiagramPropsEdgeType } from "@\/components\/settings\/TriggerTimingDiagram";\n/g, '');
        changed = true;
    }
