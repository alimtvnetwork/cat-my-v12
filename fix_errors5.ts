...
walk("src", (p) => {
  ...
  if (p.includes("sidebar.tsx")) {
    content = content.replace(
      /state === "collapsed"/g,
      "state === SidebarContextPropsStateType.Collapsed",
    );
    content = content.replace(
      /state === "expanded"/g,
