  ...
  f.forEachDescendant((node) => {
    if (Node.isUnionTypeNode(node)) {
      ...
      if (
        types.length > 0 &&
        types.every(
          (t) => Node.isLiteralTypeNode(t) && t.getFirstChildByKind(SyntaxKind.StringLiteral),
        )
      ) {
        // Collect string values
        const members = types.map((t) =>
          t.getFirstChildByKind(SyntaxKind.StringLiteral)!.getLiteralText(),
        );
        ...
        if (Node.isTypeAliasDeclaration(parent)) {
          ...
          const grandParent =
            parent.getFirstAncestorByKind(SyntaxKind.InterfaceDeclaration) ||
            parent.getFirstAncestorByKind(SyntaxKind.TypeAliasDeclaration);
          let prefix = grandParent ? grandParent.getName() : "";
          if (prefix === "Props") {
            const comp =
              parent.getFirstAncestorByKind(SyntaxKind.FunctionDeclaration) ||
              parent.getFirstAncestorByKind(SyntaxKind.VariableDeclaration);
          ...
          const propName = parent.getName();
          const prefix =
            parent.getFirstAncestorByKind(SyntaxKind.ClassDeclaration)?.getName() ||
            f.getBaseNameWithoutExtension();
          enumName = pascalCase(prefix + " " + propName) + "Type";
          enumsToCreate.push({ file: f, name: enumName, members, replaceNode: node });
          ...
          enumName =
            pascalCase(prefix + " " + (paramName.startsWith("{") ? "Param" : paramName)) + "Type";
          enumsToCreate.push({ file: f, name: enumName, members, replaceNode: node });
        }
      }
    }
...
for (const e of enumsToCreate) {
  console.log(
    `Will create: ${e.name} in ${e.file.getBaseName()} with members ${e.members.join(", ")}`,
  );
}
