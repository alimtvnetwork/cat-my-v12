    ...
    <Card className="shadow-md border-zinc-200 dark:border-zinc-800 bg-white/50 dark:bg-zinc-950/50 backdrop-blur-sm">
      ...
      <CardContent>
        <div className="space-y-6">
          <div>
            <h4 className="text-sm font-medium mb-3 text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Inputs</h4>
            <div className="grid grid-cols-2 gap-4">
              {Object.entries(inputs).map(([key, value]) => (
                <div key={key} className="flex items-center space-x-3 rounded-lg border border-zinc-200 dark:border-zinc-800 p-3 bg-zinc-50 dark:bg-zinc-900/50 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
                  ...
                  />
                  <Label htmlFor={`switch-input-${key}`} className="text-sm font-medium capitalize cursor-pointer">
          ...
          <div>
            <h4 className="text-sm font-medium mb-3 text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Outputs</h4>
            <div className="grid grid-cols-2 gap-4">
              {Object.entries(outputs).map(([key, value]) => (
                <div key={key} className="flex items-center space-x-3 rounded-lg border border-zinc-200 dark:border-zinc-800 p-3 bg-zinc-50 dark:bg-zinc-900/50 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
                  ...
                  />
                  <Label htmlFor={`switch-output-${key}`} className="text-sm font-medium capitalize cursor-pointer">
                    {key}
                  </Label>
                </div>
              ))}
