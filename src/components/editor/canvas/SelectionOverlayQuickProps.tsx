    ...
    >
          ...
          >
            <span
              className={`h-1.5 w-1.5 rounded-full ${
                hudFollowsShape ? "animate-pulse bg-[var(--ca-select,#8b5cf6)]" : "bg-ca-ink-muted"
              }`}
            />
      ...
      <div className="flex flex-col gap-2.5 border-y border-ca-border/60 bg-ca-panel/40 px-4 py-3">
              ...
              <div className="flex flex-1 gap-0.5 rounded-lg border border-ca-border/60 bg-ca-panel/60 p-0.5">
                {presets.map((preset) => (
                  <button
                    ...
                    onClick={() => updateParams(rule.id, applyPresetParams(params, preset))}
                  >
                    {preset.label}
                  </button>
                ))}
        ...
        {(() => {
          ...
          const isOverride = perRuleSnap !== undefined && perRuleSnap !== rotationSnapDefault;
      ...
      <div className="flex flex-col gap-3.5 px-4 py-4">
        {rows.map((spec) => {
          ...
          return (
            ...
            >
              <span className="text-[11px] font-medium text-ca-ink-muted">{spec.label}</span>
              <input
                type="range"
                min={spec.min}
                max={spec.max}
              ...
              />
              <span className="text-[10px] font-medium text-ca-ink-muted">{spec.suffix ?? ""}</span>
            </label>
          );
        })}
      </div>
