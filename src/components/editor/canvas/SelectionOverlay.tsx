...
// Small inline numeric editor used by the X·Y and W×H badges. Renders a
// chip that the user can double-click to type a new integer value. Commits
// on Enter or blur; Esc discards. Kept local to the overlay so it can
// share the badge chrome (border color, monospace, tabular-nums).
function BadgeNumberField({
  value,
  onCommit,
  ariaLabel,
  min,
  ...
  // so we never allow off-canvas values through a degenerate range.
  ...
  const commit = () => {
    ...
    if (Number.isFinite(n) === false) {
      // Non-numeric / empty: reject, flash invalid, keep editing so the
      // user can correct without losing focus.
      setInvalid(true);
    ...
    if (clamped !== rounded) {
      // Clamped: reflect the clamped value in the draft and flash so the
      // user sees we snapped their input to the valid range.
      setInvalid(true);
      window.setTimeout(() => setInvalid(false), 600);
    }
  ...
  if (editing && !disabled) {
    return (
      <input
        ...
        value={draft}
        onChange={(e) => {
          // Only allow an optional leading '-' and digits. Strip anything
          // else (letters, decimals, exponents, spaces) as the user types.
...
const KIND_ORDER: readonly EditorRuleKind[] = [
...
const OUTER_BLUR_PX = 6;
const INNER_BAND_BLUR_PX = 2;
...
const HANDLES: Array<{ id: string; cursor: string; sx: number; sy: number }> = [
...
function RunOrderQuickBar({
  ...
  const commit = () => {
    ...
    if (clamped - 1 !== currentIndex) onJump(clamped - 1);
    setDraft(String(clamped));
  };
