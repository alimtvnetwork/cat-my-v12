...
// determines the rule's verdict + reasonCode. Enforces the schema's
...
export async function evaluateRule(
  ...
  if (!rule.conditions || rule.conditions.length === 0) {
    return {
      ...
      reasonCode: ReasonCode.RuleConditionEval,
      conditions: [],
    };
  }
  ...
  let ruleReason: RuleResult["reasonCode"] = ReasonCode.OK;
  ...
  for (const cond of rule.conditions) {
    ...
    if (r.verdict !== VerdictType.Pass && ruleVerdict === VerdictType.Pass) {
      ...
      ruleReason = r.reasonCode;
    }
  }

  return { ruleId: rule.id, verdict: ruleVerdict, reasonCode: ruleReason, conditions: results };
}
