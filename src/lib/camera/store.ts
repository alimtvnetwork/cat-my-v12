...
function saveLibrary(
  storage: StorageLike,
  lib: CameraLibrary,
): { ok: true } | { ok: false; failure: CameraFailure } {
  try {
...
export function upsertCameraSettingSync(
  ...
  if (r.errors.length > 0) return { ok: false, failure: { kind: "validation", errors: r.errors } };
  const w = saveLibrary(s, r.library);

  if (w.ok === false) return { ok: false, failure: w.failure };
