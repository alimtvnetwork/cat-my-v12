"""`SamplesRepo`: mirror of `rule_facade.py` for the samples slice.
    ...
    def get_sample(self, sample_id: int) -> CatSample:
        ...
        if sample is None:
            raise AppError(
                ...
                {"sample_id": sample_id, "provider": "InMemorySamplesRepo"},
            )
        return sample
    ...
    def upsert_sample(self, sample: CatSample) -> CatSample:
        ...
        if not isinstance(sample.id, int) or sample.id <= 0:
            raise AppError(
                ...
                {"provider": "InMemorySamplesRepo", "op": "upsert_sample", "sample_id": sample.id},
            )
    ...
    def delete_sample(self, sample_id: int) -> None:
        if sample_id not in self._by_id:
            raise AppError(
                ErrorCode.E_BE_NOT_FOUND,
                f"sample {sample_id} not found",
                {"sample_id": sample_id, "provider": "InMemorySamplesRepo", "op": "delete_sample"},
            )
        del self._by_id[sample_id]


class VendorSamplesRepo:
    def list_samples(self) -> list[CatSample]:
        raise AppError(
            ErrorCode.E_SDK_INIT_FAILED,
            "VendorSamplesRepo not initialised",
            {"provider": "VendorSamplesRepo", "op": "list_samples"},
        )
    ...
    def get_sample(self, sample_id: int) -> CatSample:
        raise AppError(
            ErrorCode.E_SDK_INIT_FAILED,
            "VendorSamplesRepo not initialised",
            {"provider": "VendorSamplesRepo", "op": "get_sample", "sample_id": sample_id},
        )
    ...
    def upsert_sample(self, sample: CatSample) -> CatSample:
        raise AppError(
            ErrorCode.E_SDK_INIT_FAILED,
            "VendorSamplesRepo not initialised",
            {"provider": "VendorSamplesRepo", "op": "upsert_sample", "sample_id": sample.id},
        )
    ...
    def delete_sample(self, sample_id: int) -> None:
        raise AppError(
            ErrorCode.E_SDK_INIT_FAILED,
            "VendorSamplesRepo not initialised",
            {"provider": "VendorSamplesRepo", "op": "delete_sample", "sample_id": sample_id},
        )
...
assert isinstance(InMemorySamplesRepo(), SamplesRepo), "InMemorySamplesRepo drifted"
assert isinstance(VendorSamplesRepo(), SamplesRepo), "VendorSamplesRepo drifted"


_active: SamplesRepo = InMemorySamplesRepo(seed=[
    CatSample(id=1, rule_id=1, label="SOIC pockets, 4-up", captured_at="2026-08-11T12:00:00Z"),
    CatSample(id=2, rule_id=1, label="SOIC pockets, 2-up", captured_at="2026-08-11T12:00:15Z"),
    CatSample(id=3, rule_id=1, label="SOIC partial fill", captured_at="2026-08-11T12:00:30Z"),
])


def get_samples_repo() -> SamplesRepo:
    return _active


def set_samples_repo(facade: SamplesRepo) -> None:
    global _active
    _active = facade
