...
function buildInventory(dirs) {
  ...
  for (const dir of dirs) {
    ...
    const mdFiles = allEntries.filter((e) => e.isFile() && e.name.endsWith(".md"));
...
function computeHealth(linkResult, requiredFiles, inventory) {
  ...
  if (linkResult.Total.Broken > 0) {
    ...
    score -= d;
    deductions.push(`${linkResult.Total.Broken} broken links (-${d})`);
  }
  ...
  if (requiredFiles.MissingOverview.length > 0) {
    ...
    score -= d;
    deductions.push(`${requiredFiles.MissingOverview.length} missing overviews (-${d})`);
  }
