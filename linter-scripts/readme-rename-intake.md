...
documents _which_ repo-relative paths the diff-mode intake considered
and _why_ each one was kept or skipped. With `--with-similarity` the
...
| _(unknown future tag)_ | Captured as a delete by a parser this map hasn't caught up with          | `path captured as a delete by the diff intake but provenance is unknown — treated as ignored-deleted for safety` | `provenance is unknown`              |
...
reviewer can see _why_ a path was classified as deleted. Two source
...
> rename's _new_ side (with the score) is recorded as a separate
...
- [`validate-rename-intake.py --help`](./validate-rename-intake.py)
  ...
  for CI gating; see _"Validating `rename_intake` output in CI"_
...
`score` is an integer, but its _meaning_ depends on the row's kind:
...
| _(absent / empty)_  | plain A/M/D rows                   | No rename/copy provenance to label.  |
