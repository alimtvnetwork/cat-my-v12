...
func validateFile(path string, maxLines int) []Violation {
	...
	violations = append(violations, checkAnyUnknownParams(lines, path, lang)...)
...
func checkAnyUnknownParams(lines []string, path string, lang string) []Violation {
	var violations []Violation
	if lang != "typescript" && lang != "javascript" {
		return violations
	}

	// Matches `: any,`, `: unknown)`, `: any =`, etc.
	pattern := regexp.MustCompile(`:\s*(any|unknown)\b\s*([,)=]|$)`)
	...
	for i, line := range lines {
		...
		if matches := pattern.FindStringSubmatch(line); len(matches) > 0 {
			violations = append(violations, Violation{
				...
				Rule:        "CODE-RED-026",
				Severity:    "CODE-RED",
				Message:     fmt.Sprintf("Forbidden type '%s' used in parameter. Use a strict generic or safe union.", matches[1]),
				CodeSnippet: truncate(strings.TrimSpace(line), 120),
			})
		}
	}
