...
function Invoke-Linter {
    param([string]$Label, [string]$Runner, [string]$Script, [string[]]$ExtraArgs = @())
    Write-Host "`n── $Label ──" -ForegroundColor Gray
    $linterArgs = @(Join-Path $scriptDir $Script) + $ExtraArgs
    & $Runner @linterArgs
    $rc = $LASTEXITCODE
    if ($rc -eq 0) {
        $script:passed += $Label
...
if (-not $SkipLinters) {
    ...
    Invoke-Linter "ui-backend-map"       "python3" "check-ui-backend-map.py" @("--strict-schema")
    Invoke-Linter "ui-backend-map-tests" "python3" "test-check-ui-backend-map.py"
    Invoke-Linter "ipc-examples"         "python3" "check-ipc-examples.py"
    Invoke-Linter "ipc-examples-tests"   "python3" "test-check-ipc-examples.py"
