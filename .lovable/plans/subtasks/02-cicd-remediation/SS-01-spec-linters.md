...
Restore the strictness of the markdown cross-link and folder reference linters, then fix the actual underlying broken links across the `.lovable` and `spec` documentation files. Do not blindly revert unrelated legitimate compatibility fixes.
...
- Various `.md` files in `spec/` and `.lovable/`
