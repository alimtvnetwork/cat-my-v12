...
- [ ] Policy doc lists every seed slice with explicit status
- [ ] No slice marked `facade-only` still requires store for happy-path reads
- [ ] Pending-facades README has status per item
- [ ] Team/agents have a single answer to "store or facade?"
