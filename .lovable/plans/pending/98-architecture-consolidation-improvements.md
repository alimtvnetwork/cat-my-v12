...
|----|-------|-----------|-------------|
| [SS-01](../subtasks/98-architecture-consolidation/SS-01-dual-backend-map.md) | Dual-backend ownership map | Yes | 1 session |
...
| [SS-06](../subtasks/98-architecture-consolidation/SS-06-integration-test-spine.md) | Integration test spine | After SS-01 | 2 sessions |
| [SS-07](../subtasks/98-architecture-consolidation/SS-07-onboarding-refresh.md) | Onboarding & what-to-read refresh | After SS-01, SS-02 | 1 session |
...
- [ ] `docs/architecture/runtime-map.md` exists and is linked from `README.md` (SS-01)
- [ ] `BE/README.md` reflects current router/middleware/repo state — no "skeleton only" claims (SS-02)
- [ ] `src/routes/__root.tsx` < 300 lines; boot logic extracted (SS-03)
- [ ] `ProjectEditorSections.tsx` < 300 lines or split into ≤100-line components (SS-03)
- [ ] Facade migration policy doc: which slices are facade-only vs store-allowed (SS-04)
- [ ] Single canonical rule-eval owner documented; shared kernel or import boundary enforced (SS-05)
- [ ] At least one Playwright + pytest integration test: health → rules list → envelope shape (SS-06)
- [ ] `.lovable/what-to-read.md` links architecture observations + runtime map (SS-07)
