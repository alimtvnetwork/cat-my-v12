...
1. **Agent State Logs**: Temp scratchfiles like `.lovable/temp/active-locks.json` and `XX-agent-state.md` are critical to avoid collisions. Ensure `.gitignore` explicitly covers `.lovable/temp/` before running.
...
4. **Plans Index Cleanup**: Pending plans can exist in `index.md` without existing in `.lovable/plans/pending/`. Always cross-verify the file system before asserting remaining tasks.
