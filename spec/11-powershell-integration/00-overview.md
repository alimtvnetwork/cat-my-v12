...
| Upload Scripts Spec                                  | WordPress plugin upload scripts (V1, V2, V3) — _folder pending creation_   |
| Upload V1                                            | Single-file upload via Invoke-RestMethod — _folder pending creation_       |
| Upload V2                                            | Envelope-aware upload with unwrapping — _folder pending creation_          |
| Upload V3                                            | Parallel multi-plugin deployment via Start-Job — _folder pending creation_ |
| Upload Custom                                        | Custom path deployments via `run.ps1 -u -pp` — _folder pending creation_   |
| Upload Config                                        | Authentication, headers, and fallback config — _folder pending creation_   |

---

_This spec enables consistent, reproducible builds across all fullstack projects with optimized disk usage via pnpm PnP._
