...
- `# Utility functions` → _italic gray comment_
...
.code-block-wrapper:hover {
  box-shadow:
    0 8px 32px hsl(var(--lang-accent) / 0.1),
...
.code-block-wrapper.code-fullscreen {
  ...
  box-shadow:
    0 25px 80px hsl(var(--lang-accent) / 0.25),
...
.spec-h2 {
  ...
  background: linear-gradient(
    ...
    /* Purple: 252 85% 60% */ hsl(var(--heading-gradient-to)) /* Pink: 330 85% 60% */
  );
...
background: linear-gradient(
  90deg,
  hsl(var(--primary)),
  /* Purple start */ hsl(var(--accent)) /* Pink end */
);
...
background: linear-gradient(
  ...
  /* Purple tint top-left */ transparent,
  /* Clear center */ hsl(var(--accent) / 0.05) /* Pink tint bottom-right */
);
```
...
return allFiles
  .filter(
    (f) => f.name.toLowerCase().includes(q) || (f.content && f.content.toLowerCase().includes(q)),
  )
...
_Visual rendering guide — updated: 2026-04-03_
