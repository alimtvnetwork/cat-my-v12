...
3. **Root cause** — the _why_, not just the _what_
...
> ⚠️ **CRITICAL INVARIANT:** Neither workflow may invoke `npm ci`, `npm install`, `node`, or any JS toolchain. The repository ships a `package.json` for _local_ preview tooling only — the lockfile is **not** kept in sync with `package.json` and must never be relied on in CI.
...
2. `package-lock.json` is **read-only** in this project and is _not_ regenerated on every dep change.
...
**Root cause:**  
Built-in caching in `setup-python` and `setup-go` assumes the project has a canonical dependency manifest at a discoverable path. This repo is _not_ a Python or Go project — it merely _contains_ validator scripts in those languages with zero external deps. Built-in caching is the wrong tool.
...
**Prevention rule:**  
🔴 **`release.sh` and any future CI shell scripts must remain language-agnostic.** Never invoke `node`, `npm`, `python`, or `go` from `release.sh` unless the script's _purpose_ is to build that language's artifacts. Use POSIX shell utilities (`sed`, `awk`, `grep`, `jq`, `cut`) for parsing.
