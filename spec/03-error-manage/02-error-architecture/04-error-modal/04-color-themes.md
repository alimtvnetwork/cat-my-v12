...
<AlertCircle
  className={cn(
    ...
    selectedError.level === LogLevel.Error
      ? "text-destructive"
      : selectedError.level === LogLevel.Warn
        ? "text-warning"
...
<Badge
  variant="outline"
  className="shrink-0 text-xs bg-orange-500/10 border-orange-500/30 text-orange-600 dark:text-orange-400"
>
  PHP
</Badge>
```
...
_Color Theme Reference v2.0.0 — updated: 2026-03-03_
