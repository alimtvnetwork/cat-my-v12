...
if (master.Stats.TotalIndexedCodes !== computedCodes) {
  console.error(
    `❌ Stats.TotalIndexedCodes is ${master.Stats.TotalIndexedCodes} but sum of modules is ${computedCodes}`,
  );
  failed = true;
}
...
if (master.Stats.TotalRetryableCodes !== computedRetryable) {
  console.error(
    `❌ Stats.TotalRetryableCodes is ${master.Stats.TotalRetryableCodes} but sum of modules is ${computedRetryable}`,
  );
  failed = true;
}
...
if (failed) {
  ...
  console.log(
    `✅ Master index stats are consistent (${computedCodes} codes, ${computedRetryable} retryable)`,
  );
}
