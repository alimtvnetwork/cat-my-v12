...
The critical issue: `zip.Writer.Close()` writes the **central directory** to the ZIP file, which is required for the archive to be valid. With `defer`, this happens _after_ the function returns, creating a race condition where the caller reads an incomplete file.
