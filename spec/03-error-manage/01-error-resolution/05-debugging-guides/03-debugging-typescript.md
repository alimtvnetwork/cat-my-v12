...
async function fetchWithValidation<T>(url: string): Promise<T> {
  ...
  if (!body.success) {
    throw new ApiError(body.error?.code ?? 0, body.error?.message ?? "Unknown error");
  }
...
function parseHealthResponse(data: unknown): HealthResponse {
  // Validate at runtime
  if (typeof data !== "object" || data === null || !("status" in data) || !("version" in data)) {
...
- React Coding Guidelines _(external spec)_
- Shared CLI Frontend _(external spec)_
