...
Screen: Output settings image output sd card
Related-Spec: 03-canvas.md
---

# 27 � Output settings image output sd card

## 1. One-line purpose

This screen is the primary interface for Output settings image output sd card, allowing operators to configure, inspect, or manage related settings.
...
- "Output settings image output sd card"
...
1. Button (Text: "Output settings image output sd card"): Top left title area, expected to show a dropdown menu when clicked. Enabled.
...
The operator is currently on the Output settings image output sd card screen. They likely just navigated from the main menu or a previous tool configuration screen. Their immediate goal is to review or adjust the settings specific to Output settings image output sd card. After completing the setup, they will likely click Apply/Save and return to the main run screen or proceed to the next tool in the sequence.
...
- **Primitives mapping:** This screen configures a rule of kind matching `Output settings image output sd card` logic. ROIs are standard rectangular or polygonal shapes.
