...
Screen: Output settings judgment total status
Related-Spec: 03-canvas.md
---

# 26 — Output settings judgment total status

## 1. One-line purpose
This screen is the primary interface for Output settings judgment total status, allowing operators to configure, inspect, or manage related settings.
...
**Titlebar:**
- "Output settings judgment total status"
...
1. Button (Text: "Output settings judgment total status"): Top left title area, expected to show a dropdown menu when clicked. Enabled.
...
The operator is currently on the Output settings judgment total status screen. They likely just navigated from the main menu or a previous tool configuration screen. Their immediate goal is to review or adjust the settings specific to Output settings judgment total status. After completing the setup, they will likely click Apply/Save and return to the main run screen or proceed to the next tool in the sequence.
...
- **Primitives mapping:** This screen configures a rule of kind matching `Output settings judgment total status` logic. ROIs are standard rectangular or polygonal shapes.
