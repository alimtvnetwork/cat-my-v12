...
Screen: Execute condition settings tool list a
Related-Spec: 03-canvas.md
---

# 48 — Execute condition settings tool list a

## 1. One-line purpose
This screen is the primary interface for Execute condition settings tool list a, allowing operators to configure, inspect, or manage related settings.
...
**Titlebar:**
- "Execute condition settings tool list a"
...
1. Button (Text: "Execute condition settings tool list a"): Top left title area, expected to show a dropdown menu when clicked. Enabled.
...
The operator is currently on the Execute condition settings tool list a screen. They likely just navigated from the main menu or a previous tool configuration screen. Their immediate goal is to review or adjust the settings specific to Execute condition settings tool list a. After completing the setup, they will likely click Apply/Save and return to the main run screen or proceed to the next tool in the sequence.
...
- **Primitives mapping:** This screen configures a rule of kind matching `Execute condition settings tool list a` logic. ROIs are standard rectangular or polygonal shapes.
