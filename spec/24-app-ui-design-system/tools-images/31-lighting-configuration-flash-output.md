...
Screen: Lighting configuration flash output
Related-Spec: 03-canvas.md
---

# 31 � Lighting configuration flash output

## 1. One-line purpose

This screen is the primary interface for Lighting configuration flash output, allowing operators to configure, inspect, or manage related settings.
...
- "Lighting configuration flash output"
...
1. Button (Text: "Lighting configuration flash output"): Top left title area, expected to show a dropdown menu when clicked. Enabled.
...
The operator is currently on the Lighting configuration flash output screen. They likely just navigated from the main menu or a previous tool configuration screen. Their immediate goal is to review or adjust the settings specific to Lighting configuration flash output. After completing the setup, they will likely click Apply/Save and return to the main run screen or proceed to the next tool in the sequence.
...
- **Primitives mapping:** This screen configures a rule of kind matching `Lighting configuration flash output` logic. ROIs are standard rectangular or polygonal shapes.
