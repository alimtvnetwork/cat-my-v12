...
Screen: Camera settings model shutter sensitivity
Related-Spec: 03-canvas.md
---

# 29 — Camera settings model shutter sensitivity

## 1. One-line purpose
This screen is the primary interface for Camera settings model shutter sensitivity, allowing operators to configure, inspect, or manage related settings.
...
**Titlebar:**
- "Camera settings model shutter sensitivity"
...
1. Button (Text: "Camera settings model shutter sensitivity"): Top left title area, expected to show a dropdown menu when clicked. Enabled.
...
The operator is currently on the Camera settings model shutter sensitivity screen. They likely just navigated from the main menu or a previous tool configuration screen. Their immediate goal is to review or adjust the settings specific to Camera settings model shutter sensitivity. After completing the setup, they will likely click Apply/Save and return to the main run screen or proceed to the next tool in the sequence.
...
- **Primitives mapping:** This screen configures a rule of kind matching `Camera settings model shutter sensitivity` logic. ROIs are standard rectangular or polygonal shapes.
