...
Screen: Trigger settings detail mode panel
Related-Spec: 03-canvas.md
---

# 32 � Trigger settings detail mode panel

## 1. One-line purpose

This screen is the primary interface for Trigger settings detail mode panel, allowing operators to configure, inspect, or manage related settings.
...
- "Trigger settings detail mode panel"
...
1. Button (Text: "Trigger settings detail mode panel"): Top left title area, expected to show a dropdown menu when clicked. Enabled.
...
The operator is currently on the Trigger settings detail mode panel screen. They likely just navigated from the main menu or a previous tool configuration screen. Their immediate goal is to review or adjust the settings specific to Trigger settings detail mode panel. After completing the setup, they will likely click Apply/Save and return to the main run screen or proceed to the next tool in the sequence.
...
- **Primitives mapping:** This screen configures a rule of kind matching `Trigger settings detail mode panel` logic. ROIs are standard rectangular or polygonal shapes.
