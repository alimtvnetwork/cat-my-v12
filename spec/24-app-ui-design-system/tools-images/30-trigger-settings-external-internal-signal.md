...
Screen: Trigger settings external internal signal
Related-Spec: 03-canvas.md
---

# 30 � Trigger settings external internal signal

## 1. One-line purpose

This screen is the primary interface for Trigger settings external internal signal, allowing operators to configure, inspect, or manage related settings.
...
- "Trigger settings external internal signal"
...
1. Button (Text: "Trigger settings external internal signal"): Top left title area, expected to show a dropdown menu when clicked. Enabled.
...
The operator is currently on the Trigger settings external internal signal screen. They likely just navigated from the main menu or a previous tool configuration screen. Their immediate goal is to review or adjust the settings specific to Trigger settings external internal signal. After completing the setup, they will likely click Apply/Save and return to the main run screen or proceed to the next tool in the sequence.
...
- **Primitives mapping:** This screen configures a rule of kind matching `Trigger settings external internal signal` logic. ROIs are standard rectangular or polygonal shapes.
