---
Source: assets/tools-images/37-shapetrax3-search-region-green-roi-mask-config.jpg
Screen: Shapetrax3 search region green roi mask config
Related-Spec: 03-canvas.md
---

# 37 � Shapetrax3 search region green roi mask config

## 1. One-line purpose

This screen is the primary interface for Shapetrax3 search region green roi mask config, allowing operators to configure, inspect, or manage related settings.
...
- "Shapetrax3 search region green roi mask config"
...
1. Button (Text: "Shapetrax3 search region green roi mask config"): Top left title area, expected to show a dropdown menu when clicked. Enabled.
...
The operator is currently on the Shapetrax3 search region green roi mask config screen. They likely just navigated from the main menu or a previous tool configuration screen. Their immediate goal is to review or adjust the settings specific to Shapetrax3 search region green roi mask config. After completing the setup, they will likely click Apply/Save and return to the main run screen or proceed to the next tool in the sequence.
...
- **Primitives mapping:** This screen configures a rule of kind matching `Shapetrax3 search region green roi mask config` logic. ROIs are standard rectangular or polygonal shapes.
