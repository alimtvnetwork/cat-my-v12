...
Screen: Tool catalog count features
Related-Spec: 03-canvas.md
---

# 06 — Tool catalog count features

## 1. One-line purpose
This screen is the primary interface for Tool catalog count features, allowing operators to configure, inspect, or manage related settings.
...
1. Button (Text: "Tool catalog count features"): Top left title area, expected to show a dropdown menu when clicked. Enabled.
...
The operator is currently on the Tool catalog count features screen. They likely just navigated from the main menu or a previous tool configuration screen. Their immediate goal is to review or adjust the settings specific to Tool catalog count features. After completing the setup, they will likely click Apply/Save and return to the main run screen or proceed to the next tool in the sequence.
...
- **Primitives mapping:** This screen configures a rule of kind matching `Tool catalog count features` logic. ROIs are standard rectangular or polygonal shapes.
