...
Screen: Preferred tool pattern match shading
Related-Spec: 03-canvas.md
---

# 15 — Preferred tool pattern match shading

## 1. One-line purpose
This screen is the primary interface for Preferred tool pattern match shading, allowing operators to configure, inspect, or manage related settings.
...
**Titlebar:**
- "Preferred tool pattern match shading"
...
1. Button (Text: "Preferred tool pattern match shading"): Top left title area, expected to show a dropdown menu when clicked. Enabled.
...
The operator is currently on the Preferred tool pattern match shading screen. They likely just navigated from the main menu or a previous tool configuration screen. Their immediate goal is to review or adjust the settings specific to Preferred tool pattern match shading. After completing the setup, they will likely click Apply/Save and return to the main run screen or proceed to the next tool in the sequence.
...
- **Primitives mapping:** This screen configures a rule of kind matching `Preferred tool pattern match shading` logic. ROIs are standard rectangular or polygonal shapes.
